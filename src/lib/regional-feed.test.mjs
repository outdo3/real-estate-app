...
test('annotateTrades: 신고가는 그룹 내 시간순 누적 최고가 기준(미래 거래가 과거 판정에 영향 없음)', () => {
  ...
  assert.equal(ann.get('t1').isRecordHigh, true); // 첫 거래는 항상 신고가(비교 대상 없음)
  assert.equal(ann.get('t2').isRecordHigh, false);
  assert.equal(ann.get('t3').isRecordHigh, true);
...
test('annotateTrades: 취소거래는 신고가/직전거래 계산에서 완전히 제외된다', () => {
  ...
  assert.equal(ann.get('t2').isRecordHigh, true);
});
...
test('buildRegionSummary: 취소/신고가/상승/하락 집계', () => {
  ...
  assert.equal(summary.recordHighCount, 2); // t1(최초), t3
...
test('buildMarketInterpretation: 표본이 너무 적으면 문장을 만들지 않는다(과잉해석 방지)', () => {
  ...
  const lines = buildMarketInterpretation({
    periodLabel: '최근 7일', periodDays: 7, summary, lookbackVerifiedCount: 100, lookbackDays: 365, dongCounts: {}, areaBandCounts: {},
  });
...
test('buildMarketInterpretation: 단정적 표현("확정", "적기", "급등") 없이 사실 기반 문장만 생성', () => {
  ...
  assert.ok(lines.some((l) => l.includes('신고가 거래가 4건')));
