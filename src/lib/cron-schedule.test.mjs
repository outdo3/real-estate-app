...
test('실제 등록할 SALE/RENT 표현식이 의도한 KST 시각으로 해석된다', () => {
  assert.equal(describeDailyUtcCronInKst('0 19 * * *'), '매일 04:00 KST');
  assert.equal(describeDailyUtcCronInKst('0 20 * * *'), '매일 05:00 KST');
});
...
const CRONS = [
  ...
  { path: '/api/cron/rent-sync?mode=apply', schedule: '0 20 * * *' },
];
