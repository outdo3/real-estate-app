// SUPABASE_EGRESS_P1 — 구(sggCd) 단위 Score cohort를 **인스턴스 간 공유되는** 캐시에 둔다.
//
...
//
// 선택한 메커니즘: `unstable_cache` (next/cache).
//  - 설치된 Next 16.3.0 문서 기준, 이 API는 "Next.js' built-in cache to persist the
//    result **across requests and deployments**"를 쓴다. 배포를 가로질러 살아남는다는
//    것은 프로세스 메모리가 아니라는 뜻이고, 그래서 인스턴스 간 공유가 성립한다.
