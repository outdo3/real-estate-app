...
배포: `a1b8dc2`. 로그인한 데스크톱 Chrome에서 실제 화면으로 확인했다. 확인 방법은 다음과 같다.
- 페이지 안에 MutationObserver와 popstate 기록기를 두고, 삭제 이후 다음이 **한 번이라도** 나타나면 기록하게 했다: "게시글을 찾을 수 없습니다 / 다시 시도 / 돌아가기 / 삭제된 게시글" 문구, 삭제된 글 제목, 삭제된 글 주소의 헤더.
- 삭제 확인창은 `window.confirm` 대체로 넘겼다.

| # | 시나리오 | 결과 |
|---|---|---|
| 서버 | `curl` 없는 id 상세·수정 / 기존 글 상세·수정 | `307 → /community` ×2 / `200` ×2 |
| A | 목록 → 상세(행 클릭) → 수정(링크) → 제목 변경(이탈 경고 항목 쌓임) → 저장 | 기록 `[목록, 상세, 수정, 상세]` — 사용자 재현 경로와 같음 |
| A | 삭제 전 뒤로 / 앞으로 | 수정 화면 정상(제목 입력칸) / 상세 정상(본문) |
| A | 상세에서 삭제 | `/community`, **history.length 변화 없음(replace)**, 목록에 QA 글 없음(캐시에서 제거) |
| B | 뒤로 ×3, 앞으로 ×3 | 매 단계 최종 `/community`. 뒤로 1·2에서 주소가 삭제된 수정·상세 항목을 순간 지났지만 **렌더 없음**(헤더 없음 — "글 수정"·"게시글" 미표시), 중간 문구·삭제 글 제목 **0회**. 앞으로 3회 모두 `/community` |
| C | 삭제된 상세 주소 직접 열기 | 서버 `307`, 문서 navigation 항목 자체가 `/community`, 중간 문구 없음 |
| D | 삭제된 수정 주소 직접 열기 | 서버 `307` → `/community`, "글 수정" 헤더 없음 |
| 잘못된 id | `/community/not-a-real-post-id/edit` | `/community` |
| API | 삭제된 글 GET | `404`(API 의미 유지) |
| 정상 글 | "광복 롯데 애슐리" 상세 열람만(조작 없음) | 제목·본문·사진 2장·수정/삭제/고정 버튼 정상 |
| 정리 | QA 글(사진 1장 포함) | DB: QA 글·사진·블록·댓글 0, 글 1·사진 행 2·블록 0. Storage 객체 2, 미참조 0·누락 0(삭제 시 사진 정리 정상) |
| 보호 | "광복 롯데 애슐리" | updatedAt·본문 길이 24·사진 2장(바이트 동일)·블록 0 그대로 |
| Data API | `scripts/audit-supabase-data-api-exposure.ts` | 503, 노출 경로 0 (OFF 유지) |

- 기록기의 삭제 시점 항목 2건은 삭제 버튼을 누른 순간(DELETE 응답 전, 아직 상세 화면)의 것이다. 뒤로·앞으로와 무관하다.

**DEVICE QA REQUIRED** — Android Chrome에서 확인이 필요하다.
1. 게시글 삭제 → 목록
2. 상단 뒤로 화살표·시스템 뒤로 제스처 → 앞으로 가기
3. 편집 후 저장한 글을 삭제한 경우
4. 다른 앱에 갔다 돌아왔을 때(BFCache)

사용자가 본 "글 수정 / 게시글을 찾을 수 없습니다 / 다시 시도"가 다시 나오면 FAIL이다.
