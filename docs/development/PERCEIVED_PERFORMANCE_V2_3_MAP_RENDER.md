...
### 6.1 Production AFTER (배포 9e73ab9 후, 동일 harness / 동일 조건)

| 구 | data ready | usable | **렌더 구간(usable−data)** | LTmax |
|---|---|---|---|---|
| 중구 | 2,469ms | 3,540ms | 1,071ms | 112ms |
| 부산진구 | 2,640ms | **3,797ms** | **1,157ms** | **207ms** |
| 해운대구 | 2,567ms | **3,521ms** | **954ms** | **99ms** |

| 구 | LTmax BEFORE→AFTER | usable BEFORE→AFTER | 렌더 구간 BEFORE→AFTER |
|---|---|---|---|
| 중구 | 143 → 112ms | 3,582 → 3,540ms | 1,166 → 1,071ms |
| 부산진구 | **862 → 207ms (−76%)** | 4,250 → 3,797ms (−453ms) | 1,736 → **1,157ms (−579ms)** |
| 해운대구 | **658 → 99ms (−85%)** | 4,090 → 3,521ms (−569ms) | 1,508 → **954ms (−554ms)** |

### 6.2 §8 목표 대비 — 정직한 판정

- **"밀집 구 렌더 단계를 현재 약 2.4초보다 확실히 아래로"** → **달성.**
  마커 렌더 long task가 862 → 207ms(부산진구), 658 → 99ms(해운대구)다.
- **"부산진구 usable ≤3s"** → **이 측정 조건에서는 미달(3,797ms).**
  이유는 렌더가 아니라 **data ready가 이미 2,640ms**이기 때문이다. 이 조건은
  Slow 4G(1.6Mbps/150ms RTT) + CPU 4x + cold cache로 일부러 가혹하게 잡은 값이다.
  사용자가 보고한 조건(응답 약 1.1초)에서는 같은 렌더 개선이 usable을 약 2.3초
  부근으로 옮기지만, **그건 추정이며 이 문서가 측정한 값이 아니다.**
- 결론: **렌더 병목은 해소됐고, 남은 지배 비용은 data ready로 되돌아갔다** —
  그건 이 STEP의 범위(렌더)가 아니라 데이터 경로 STEP의 문제다.
...
- **P0 없음.** 렌더 경로에서 확인된 미해결 결함은 없다.
- **다음 지배 비용은 data ready(production 2.6초)** — 렌더가 아니라 데이터 경로다.
  `/api/transactions?months=12` 응답과 그 앞단(cold start/역지오코딩)이 대상이며
  별도 STEP이 필요하다.
