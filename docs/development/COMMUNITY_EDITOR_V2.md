...
| 항목 | 결과 |
|---|---|
| `src/lib/community/community-editor-v2.test.ts` | 31/31 (요청 42항목 매핑 + 모바일 CSS·이탈 경고·스냅샷·관련 단지·저장 후 캐시 갱신) |
| 커뮤니티 기존 테스트(V1 이미지·launch·익명 열람) | 68/68 — 위치가 옮겨진 3개 검사(세션 정리·상세 select·사진 렌더)만 새 위치로 갱신 |
| src 전체 | **1843/1843** (QA 결함 회귀 테스트 1개 추가 후) |
...
### 7.1 Production QA (e-jip.com, 데스크톱 Chrome, 로그인된 브라우저)

배포: `ee37cbc`(07:25:29 UTC) → 수정 `1b44f60`(07:33:30) → 재수정 `1ace3ec`(07:39:03). 실제 배포 화면의 편집기 버튼으로 조작했다
(파일 선택창만 page JS로 대체해 같은 `<input type=file>`에 합성 사진 파일을 넣음, `confirm`은 자동 확인). QA 글은 검증 후 삭제.

| 확인 | 결과 |
|---|---|
| legacy 글 읽기(애슐리, read-only) | 상세 API blocks = `[text(24자), image 1200×1600, image 1200×1600]`, updatedAt 불변 |
| 작성: 글 → 사진A(빨강 표식) → 글 → 사진B(파랑) | 편집기 `사진 2/5 · 블록 4/25`, 등록 성공 |
| 작성 중 사진·글 블록 삭제 버튼 | 여분 사진 블록·빈 글 블록 삭제 동작 |
| 상세 렌더 / 새로고침 / 익명 API | 모두 `TEXT → IMAGE(빨강) → TEXT → IMAGE(파랑)`, 파생 content = 두 글을 빈 줄로 연결, lazy·저장 크기 |
| 수정 진입 | 작성자에게 수정 링크, 저장 블록·제목 그대로 preload, 변경 전 저장 버튼 비활성 |
| 수정: 사진A 삭제 → 사진B 위로 → 글1 수정 → 맨 위에 새 사진C(초록) | 저장 성공 |
| 수정 결과(서버·익명·새로고침) | `IMAGE(C, 새 사진) → TEXT(수정됨) → IMAGE(B, **같은 imageId 유지**) → TEXT` |
| 제거 사진A | 캐시 우회 공개 URL 400(Storage 삭제), 유지 사진B 200 |
| 동시 수정 | 이전 `updatedAt`으로 PATCH → **409** + 안내 문구, 글 불변 |
| DB | 블록 4개 모두 이 글의 사진만 참조, 사진 순번 = 블록 안 등장 순서, Storage 참조 안 되는 객체 0 |
| 삭제 | 200, 상세 404, 사진 캐시 우회 400, 전체 posts 1 / post_images 2 / blocks 0 / Storage 객체 2(애슐리 글), 미참조 객체 0 |
| 애슐리 글 | 적용 전 스냅샷과 id·updatedAt·본문 길이·사진 2행(id, 순번, bytes, 크기) 동일, 블록 0 |
| Data API | OFF 유지(서버 키로도 503) |

### 7.2 QA에서 발견해 고친 결함

**수정 저장 직후 상세 화면이 수정 전 내용을 보여줌.** 서버 데이터는 정상이었고(새로고침하면 정상), 상세 화면의 SWR 캐시가 원인이었다.
- 1차 수정(`1b44f60`): 이동 전에 `mutate(key)` 호출 → Production 재현 시 여전히 실패.
  SWR 2.5 `internalMutate`는 데이터 없이 호출되면 **마운트된 훅만** 재검증한다(소스 확인). 수정 화면에서는 상세가 언마운트 상태라 캐시가 그대로였다.
- 2차 수정(`1ace3ec`): 저장된 글을 받아 **캐시에 직접 넣은 뒤** 이동(`mutate(key, fetch(...), { revalidate: false })`).
  Production에서 같은 시나리오(상세 → 수정 링크 → 순서 변경 → 저장) 직후 새로고침 없이 새 순서 표시 확인.
- 테스트로 고정: 캐시에 데이터를 넣는 형태와 "갱신이 이동보다 먼저"를 검사.

### 7.3 측정하지 못한 것

- 360/375/390px 레이아웃, 키보드가 올라온 상태, Android 시스템 뒤로 제스처와 이탈 경고: 자동화 환경(데스크톱 창·백그라운드 탭)으로 확인 불가.
- 사진 처리 체감 속도: 자동화 탭이 백그라운드라 타이머·rAF가 제한돼 측정 무효.

### 7.4 기기 QA (필수, 미실시)

Android Chrome: 글부터 → 사진 → 글 / 사진부터 → 글 / 사진 여러 장 한 번에 / ↑↓ 이동 / 수정 진입 / 기존 사진 삭제 / 새 사진 추가 /
저장 후 순서 유지 / 키보드 올라온 상태에서 입력·등록 버튼 / 취소·뒤로 가기 경고.
