...
## 11.5. Production QA (배포 후, §22)

커밋 `48999e9` push → Vercel Production 배포 완료.
...
| `/stats` | 200 | 0.88s | 없음 |
| `/report` | 200 | 0.36s | 없음 |
| `/report/city/busan` | 200 | 2.25s | 없음 |
| `/report/district/26140` | 200 | 0.30s | 없음 |
| `/school` | 200 | 0.11s | 없음 |
| `/redevelopment` | 200 | 0.28s | 없음 |
| `/apt/...?lawdCd=26140` | 200 | 0.32s | 없음 |
| `/api/admin/ops` (무인증) | **401** | — | 게이트 정상 |

**registry 파생이 Production에서 실제로 동작한다는 직접 증거**: `/report` 페이지가 렌더한 지역 코드가 정확히 16개이고 기존 값과 동일하다.
...
| 확인 | 결과 |
|---|---|
| sitemap `<loc>` | **139** (배포 전과 동일) |
| sitemap 내 서울/경기 URL | **0** |
| 5xx | **0** |
| `error_logs` 최근 2시간 / 24시간 | **0 / 0** (최신 항목 2026-09-11, 배포 5일 전) |

`/stats`·`/school`의 시군구 셀렉트는 종전처럼 `REGION_DATA`(전국 이름)를 쓴다 — 이번 STEP에서 그 파일을 건드리지 않았으므로 **노출 범위 변화가 없다**.
