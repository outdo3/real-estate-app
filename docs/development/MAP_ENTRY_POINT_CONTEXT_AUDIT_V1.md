...
## 7. Production QA (배포 `d03ea36`, 2026-09-15)

GPS 없는 데스크톱 환경(일반 흐름이면 GPS 실패 → IP 접속 지역 26110 + 안내가 **정상**). 이동 후 약 12~14초 뒤 확인.

| 유형 | 경로 | 최종 지도 | 선택 | 안내 · ipinfo | 뒤로가기 |
|---|---|---|---|---|---|
| 수정(일반) | 연서초등학교 상세 → 헤더 "지도" | `lat=35.1017&lng=129.03&lawdCd=26110`(일반 흐름, 학교 좌표 아님) | — | 표시 · 요청 | 1회 → `/school/21335731?…` |
| 수정(일반) | 부산외국어고등학교 상세 → 헤더 "지도" | 26110 일반 흐름 | — | 표시 · 요청 | — |
| 일반 | 우동 롯데 상세 → 헤더 "지도" | 26110 일반 흐름, aptSeq 없음 | — | 표시 · 요청 | 1회 → `/apt/롯데` |
| 일반 | 홈 "지도에서 찾기"(Link) | 26110 일반 흐름, 마커 26140→26110 | — | 표시 · 요청 | 1회 → `/` |
| 단지 | 연산자이 상세 → 지도에서 주변 단지와 보기 | `lat=35.1879046843468&lng=129.089787157079&lawdCd=26470&aptSeq=26470-1066` | 연산자이 카드 | 없음 · 없음 | — |
| 단지 | 우동 롯데 상세 → 같은 버튼 | `lat=35.1634441587193&lng=129.147619699842&lawdCd=26350&aptSeq=26350-9` | 롯데 카드 | 없음 · 없음 | 1회 → `/apt/롯데?…aptSeq=26350-9` |
| 지역 | 홈 검색 연산동 | `lat=35.1825602536452&lng=129.085536368165&lawdCd=26470`, 마커 26470 | — | 없음 · 없음 | 1회 → `/` |
| 지역 | 홈 검색 화명동 | `lat=35.2357636793361&lng=129.013861621487&lawdCd=26320`, 마커 26320 | — | 없음 · 없음 | — |
| 지도 내부 | (연산자이 지도) 하단 카드 "상세보기" → 상세 → back | 같은 `lat/lng/lawdCd=26470/aptSeq=26470-1066`로 복원 | 연산자이 카드 유지 | 없음 | — |

- 수정 전 같은 학교 경로는 학교 좌표·26470·안내 없음으로 열렸다(§3) → 수정 후 다른 일반 진입과 같은 결과.
- 뒤로가기 경로 모두 1회에 출발 화면 복귀, history 중복 없음.
- 오피스텔·재개발·분양·비교에는 지도 진입이 없어 D 유형 QA 대상 없음.
