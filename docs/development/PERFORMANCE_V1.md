...
- Vercel production numbers for Score/area84 (single read-only spot-check) were noticeably higher than local PROD-LOCAL numbers (e.g., Score cold ~11.8s vs ~1.5–2.9s locally) even though the deployment region (`icn1`) matches the DB region — consistent with the well-known Prisma+PgBouncer prepared-statement re-negotiation cost under serverless connection churn. This was **not** touched (a `DATABASE_URL`/pooler-mode change is a production infra change outside this STEP's safe-autonomous scope) but is flagged here as the most likely reason local and production numbers diverge, worth investigating in a dedicated future STEP before assuming today's local measurements fully represent production.
  **CORRECTED (2026-09-02, `PERFORMANCE_V1.3`):** the "deployment region matches the DB region" premise above was wrong — it was inferred from the `X-Vercel-Id` header's `icn1::...` prefix, which is the **edge/CDN** location, not the function's execution region. `vercel inspect` on the live production deployment shows every Lambda function actually builds and runs in **`iad1`** (US East, Virginia), while the database is confirmed in `ap-northeast-2` (Seoul) — a full trans-Pacific round-trip (~300-500ms) on every DB query, not a PgBouncer prepared-statement cost. This is now understood to be the dominant root cause across nearly all of this document's "Vercel slower than local" observations. See `PERFORMANCE_V1_3_VERCEL_REALITY_CHECK.md` for the full investigation and the recommended (not yet applied) fix.
  **RESOLVED (2026-09-02, `PERFORMANCE_V1.4`):** user approved the region fix. Root `vercel.json` (`{"regions": ["icn1"]}`) added, committed (`35172b8`), and deployed. `vercel inspect` on the resulting deployment confirms every Lambda function now builds and runs in `icn1`, and the live canonical URL's `X-Vercel-Id` now reads `icn1::icn1::...` (previously `icn1::iad1::...`). Full 12-route production re-benchmark: every route improved, all previously-P0/FAIL routes (Search, both dashboards, area84, Rise, Decline, Record High) are now PASS or FAST, and zero routes remain above 3s. Only the Busan-wide dashboard's cold run (2.90s, down from 16.25s) remains above 1.5s, and that residual is attributed to its own separately-tracked MOLIT external-call dependency (`RENT_TRADE_HISTORY_V1_PHASE_D2`), not region. This closes the region-mismatch risk. See `PERFORMANCE_V1_4_VERCEL_REGION.md` for full before/after numbers.
