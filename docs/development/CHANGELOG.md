...
작성기 UI 상태만 변경. 상세: `docs/development/COMMUNITY_EDITOR_V2_1A_PHOTO_REPLACE_INSERTION_FIX.md`
    ...
    수정      removeComposerImageWithAnchor: 삭제 자리를 커서로 반환(합쳐진 글은 뒤 글 시작 위치 → 교체 시 앞뒤 글 원래대로),
              커서 기억에 source(user | delete-anchor), pickComposerInsertCursor: 선택 사진 > 삭제 anchor > 포커스 입력칸 > 사용자 커서,
              anchor는 재읽기 제외, select·blur는 anchor를 덮지 않음(QA에서 남은 포커스 입력칸이 anchor를 이기는 것 발견 → 보강).
...
파트너 데이터, 노출 조건(lawdCd + 법정동 정확 일치), 집계 훅, 이벤트 이름, API 변경 없음.

    구조     [지역 중개 상담] → h2 "이 지역 중개가 필요하신가요?" → 상호(700) + 대표 공인중개사
             → 전화 051-714-2225(본문보다 한 단계 크게) → 상담 범위(설정 문구 그대로)
             → 전화 상담 버튼 → 구분선 아래 중개사무소 등록번호 · 주소(작게)
    표면     #f1f5f9 blue-gray, 1px #d9e2ec, radius 16px, 아주 약한 그림자. 브랜드 초록/원색/그라데이션/애니메이션 없음
    CTA      "전화 상담" 1개(짙은 슬레이트 채움, 48px). 문의 채널이 전화뿐이라 "중개 문의" 버튼은 만들지 않음
    모바일   세로 쌓기, 버튼 전체 폭. 390px: 제목·상호·번호·등록번호 각 1줄, 360px: 상담 범위만 2줄, 가로 넘침 없음
    데스크톱 768px 이상 텍스트 왼쪽 / 버튼 오른쪽(180px)
    접근성   semantic h2, tel 링크 aria-label, focus-visible 외곽선, 색이 아닌 텍스트로 의미 전달
    집계     usePartnerCta 그대로(partner_cta_impression / partner_cta_click, channel=phone). 새 이벤트 없음

정책 변경(사용자 요청): 예전(APT_DETAIL_PARTNER_TRADE_DENSITY_V1 §4/§9)에는 번호를 화면·aria에서 감춰 모든 통화가
집계 버튼을 거치게 했다. 이제 중개사 카드는 번호를 **텍스트로** 보인다 — 번호를 보고 직접 거는 통화는 집계되지 않는다.
탭해서 거는 경로는 집계 버튼 하나뿐이다(번호는 링크가 아님). 법무사 카드는 그대로 비노출.

검증: 실제 서버 렌더 테스트 3개 + 계약 테스트 갱신, src 전체 1765/1765, tsc FAIL_EXISTING_SCRIPT_ERRORS(src 0),
eslint exit 0, build exit 0. 로컬 프로덕션 빌드를 브라우저로 390/360px(iframe)·767px·1280px에서 측정/확인.
DEVICE QA REQUIRED.
...
기간 버튼을 바꾸면 조회가 진행 중인데도
    ...
    부산 …, 최근 12개월 기간 내 실거래가 없어요

가 잠깐 스쳤다. 계산 문제가 아니라 분기 문제였다.
...
`!data`는 "없다"가 아니라 **"아직 안 왔다"**다. 기간/지역을 바꾸면 SWR 키가 바뀌고
그 순간 data는 undefined가 되는데, 그걸 0건과 한 조건으로 묶어 빈 상태를 먼저 렌더했다.

신고된 화면(실거래 피드)은 한 겹 더 있었다. 로딩 분기가

    isLoading && offset === 0 && visibleTrades.length === 0

이라서, 이전 목록이 남아 있는 전환 중에는 로딩으로 가지 못하고 곧바로 아래 `!data`에
걸렸다. 즉 깜빡임 방지용으로 이전 목록을 유지하려던 장치(§16)가 오히려 빈 상태
문구를 띄우는 경로를 만들고 있었다.

고친 것

상태 우선순위를 순수 모듈 하나(src/lib/stats/view-state.ts)로 정의했다.

    오류 → 로딩 → 빈 상태 → 성공

규칙 두 개를 코드로 고정했다. 오류는 로딩보다 먼저 판단한다(실패를 로딩으로 붙잡으면
무한 스피너가 된다). 그리고 응답이 없으면 fetching이 아니어도 빈 상태가 아니다 —
모르는 상태를 "없음"으로 단정하지 않는다.

화면 6곳의 분기를 정리했다. 실거래 피드, 거래집중, 공급, 갭투자, 가격순위(상승/하락/
2년최고가/전세가하락), 84㎡ 순위. 응답이 없으면 로딩 문구를 보여주고, 빈 상태는
응답이 도착한 뒤에만 말한다.

덤으로 발견한 캐시 버그

가격순위와 84㎡ 순위는 목록을 SWR onSuccess로만 누적했다. onSuccess는 실제 fetch에서만
불리므로 dedupingInterval(60초) 안에 같은 조건으로 되돌아오면 호출되지 않는데, 조건이
바뀔 때 누적본을 비우기 때문에 그 경우 data는 정상인데 목록만 비어 "거래가 없어요"가
떴다. 실거래 피드가 PERFORMANCE_V2 §10에서 이미 고친 것과 같은 원인이다. 누적본이
비어 있으면 현재 응답에서 직접 가져오도록 했다(누적본이 있으면 그 쪽이 이겨서
더보기로 쌓은 페이지를 잃지 않는다).

건드리지 않은 것

지역 변동지도는 이미 정상이었다 — 조회 시작과 동시에 로딩을 켜고, 수동 fetch에
cancelled 가드가 있어 stale 응답이 최신 선택을 덮지 않는다. 거래량 카드도 `!data`를
오류로 처리하는 게 맞는 구조였다(응답이 왔고 success가 false인 경우). 둘 다 그대로 뒀다.

통계 계산식, API 경로/파라미터, 누적 헬퍼, 캐릭터·이미지 자산은 바꾸지 않았다.
새 스켈레톤이나 애니메이션도 추가하지 않았고 기존 InlineLoading / Empty / ErrorState를
그대로 쓴다. 추가 API 호출도 polling도 없다.

테스트

    npx tsx --test src/lib/stats/view-state.test.ts   18/18 PASS
    npx tsx --test "src/**/*.test.ts"                 986/986 PASS
    npx tsc --noEmit                                  src/ 오류 0
    npm run build                                     Compiled successfully

브라우저가 없어 360/390/430px 실제 렌더와 기간 버튼 연타 체감은 확인하지 못했다 —
STRUCTURAL PASS / DEVICE QA REQUIRED.

## 2026-09-12

### RECENT VIEWED AUTH PARITY FIX V1 — 최근 본 단지 로그인 분리

신고된 증상은 "로그인해도 홈의 최근 본 단지가 로그인 전과 같다"였는데, 감사해보니
표시 버그가 아니라 **기록 유출**이었다.

원인 세 갈래

    1. 상세페이지 방문 기록을 로그인 여부와 무관하게 localStorage에 썼다
    2. useRecentSync가 서버(계정) 목록을 localStorage에 mirror했다
    3. 홈은 세션을 보지 않고 그 localStorage만 읽었다(마운트 시 1회)

그래서 A 계정으로 로그인해 단지를 둘러보고 로그아웃하면, 그 기록이 "게스트 기록"으로
홈에 그대로 남았다. 같은 기기를 쓰는 다른 사람에게도, B 계정으로 로그인해도(B가 MY에
들어가 sync를 돌리기 전까지) A가 본 단지가 먼저 보였다. useRecentSync는 /my에서만
호출되므로 홈은 sync 전까지 계속 옛 local을 보여줬다 — 신고 증상 그대로다.

정책대로 분리

    로그인   → 서버(계정) 기록만. local은 읽지 않는다
    비로그인 → 이 기기에서 비회원으로 본 기록만
    세션 확인 중 → 아무것도 보여주지 않는다(잘못된 목록 flash 금지)

새 훅 useRecentApartments가 출처를 가른다. SWR 키에 사용자 id를 넣어(#userId) A의
응답이 B 화면에 캐시로 재사용될 수 없게 했고, 로그아웃하면 키가 null이 되어 계정
목록이 즉시 화면에서 빠진다(늦게 도착한 응답도 다른 키라 렌더되지 않는다).

유출 경로 차단

localStorage 키를 ejip:recentApartments:guest:v2로 올렸다. 코드를 고쳐도 이미 사용자
브라우저에 남은 오염된 값은 사라지지 않기 때문이다 — 새 키를 읽고, 읽는 순간 옛 키를
지운다. 그리고 로그인 상태의 방문은 게스트 저장소에 쓰지 않고, useRecentSync의
서버 → local mirror를 제거했다(업로드 = local 기록을 계정에 병합하는 경로는 유지).

상세페이지의 게스트 기록은 별도 effect로 분리했다. 기존 로그 effect는 pageReady가
확정되는 순간 한 번 돌고 세션 상태를 deps에 담지 않아서, 세션이 아직 'loading'인 채
그 시점이 지나가면 로그인 사용자의 방문이 게스트 기록으로 남을 수 있었다. 이제
세션이 확정되고 비로그인일 때만 쓴다 — 모르는 상태를 게스트로 단정하지 않는다.

서버 데이터는 지우지 않는다. 로그아웃은 화면에서만 사라지고, 같은 계정으로 재로그인하면
그대로 복구된다(테스트로 고정).

실패 처리

로그인 상태에서 /api/my/recent가 실패하면 local로 대체하지 않는다. 남의 목록을 내
기록인 척 보여주는 것보다 "최근 본 단지를 불러오지 못했어요"가 낫다.

홈 개수

기본 5개 + 초과 시 더보기/접기. 개수 규칙은 MY의 최근 목록과 같은 모듈
(src/lib/my/recent-rows.ts)을 쓴다 — 홈이 자기만의 상한(HOME_RECENT_LIMIT)을 따로
두지 않게 했다. 로그인/비로그인 동일 UX.

merge 정책

로그인 시 local → 서버 병합은 이미 있던 로직(useRecentSync)이고 그대로 뒀다.
자동 merge UX를 새로 만들지 않았다 — 기본은 로그인 시 계정 기록 우선이다.

테스트

    npx tsx --test src/lib/recent-auth-parity.test.ts   20/20 PASS
    npx tsx --test "src/**/*.test.ts"                   968/968 PASS
    npx tsc --noEmit                                    src/ 오류 0
    npm run build                                       Compiled successfully

브라우저가 없어 360/390/430px 실제 렌더와 로그인/로그아웃 전환은 확인하지 못했다 —
STRUCTURAL PASS / DEVICE QA REQUIRED.

남은 항목(P2): 상세페이지 빠른검색(ApartmentQuickSearch)은 여전히 게스트 저장소를
직접 읽는다. 이제 그 저장소에는 비회원 기록만 남으므로 유출은 없지만, 로그인
사용자에게는 계정 기록과 다른 목록이 보인다. 별도 STEP에서 같은 훅으로 통일할 수 있다.
...
없었다. MY 프로필 카드에 인라인 편집을 붙였다. 스키마 변경 없음.

감사에서 세 가지가 먼저 확인됐다

첫째, 스키마를 바꿀 필요가 없다. User.name은 이미 NOT NULL이고 unique가 없다 —
중복 닉네임이 현재 정책이라 임의로 unique를 붙이지 않았다. 붙였다면 이미 같은 이름을
쓰는 기존 사용자가 저장에 실패한다.

둘째, 커뮤니티는 작성자명을 snapshot하지 않는다. Post/Comment는 authorId만 저장하고
조회할 때 User를 join해 name을 가져온다. 그래서 닉네임을 바꾸면 **기존 글과 댓글의
작성자명도 자동으로 따라온다.** bulk update가 필요 없고 하지 않았다.

셋째, 가장 중요한 위험이었던 재로그인 overwrite는 **이미 안전했다.** 설치된 next-auth
v4의 callback-handler를 직접 읽어 확인했다 — 이미 연결된 계정으로 다시 로그인하면
userByAccount를 그대로 반환하고 updateUser를 부르지 않는다. createUser는 신규 가입
때만 돈다. 보호 장치를 새로 만들지 않았고, 대신 이 전제가 깨지면 드러나도록 테스트로
고정했다.

검증 규칙은 최소한으로

trim, 빈 값 거부, 2~20자, 제어문자 거부. 금칙어 사전은 만들지 않았다 — 운영 기준
없이 만든 필터는 멀쩡한 이름을 막고 막아야 할 것은 못 막는다.

길이는 코드 포인트로 센다. '집🏠'.length는 3이지만 사용자가 보는 글자는 2개라,
.length를 쓰면 이모지 두 개짜리 닉네임이 "4자"로 거부된다.

제어문자 판정은 정규식 문자 클래스가 아니라 코드 포인트 비교로 했다. 소스에
제어문자를 리터럴로 적으면 에디터/도구마다 다르게 보이고 diff가 조용히 깨진다.

API

PUT /api/my/profile — 기존 /api/my/preferences 패턴 그대로. userId는 항상
requireUser()가 돌려준 세션 사용자에서만 오고, 요청 본문의 사용자 식별자는 신뢰하지
않는 정도가 아니라 아예 읽지 않는다. 인증 확인이 본문 파싱보다 먼저다. 이 라우트는
name만 쓰며 이메일·이미지는 코드에 등장조차 하지 않는다. 실패 로그에는 정적 문자열
하나만 찍는다.

세션 갱신

JWT 전략이라 token.name은 로그인 시점에 구워진다. DB만 바꾸면 토큰이 갱신될 때까지
헤더와 MY에 옛 이름이 남는다. 저장 후 useSession().update()를 부르고, jwt 콜백에
trigger === 'update' 분기를 추가했다.

update()가 넘긴 값을 쓰지 않고 DB에서 다시 읽는다. update()의 인자는 클라이언트가
정하는 값이라 그대로 토큰에 넣으면 저장하지 않은 이름을 자기 세션에 표시할 수 있다.
DB를 진실로 삼으면 그 경로가 없다. 그래서 클라이언트도 인자 없이 호출한다.
조회는 명시적 update 때만 일어나므로 평소 인증 비용은 그대로고, 실패해도 삼켜서
로그인 상태를 깨뜨리지 않는다.

이 변경은 authOptions.callbacks.jwt를 건드린다. 추가만 했고(기존 분기 그대로),
명시적 update 트리거에서만 동작하며, 세션 전략·provider·계정 연결 로직은 손대지 않았다.

UI

프로필 카드 인라인 편집. 모달을 새로 만들지 않았다. 저장 중 비활성화, Enter 저장 /
Escape 취소, 오류는 role="alert" 성공은 role="status"로 인라인 표시. 입력은
font-size 16px — 그보다 작으면 iOS·안드로이드가 자동 확대를 걸어 키보드가 열릴 때
화면이 튄다. 이메일은 읽기 전용이고 프로필 사진 수정 기능은 만들지 않았다.

브라우저가 없어 360/390/430px 실제 렌더는 확인하지 못했다 — DEVICE QA REQUIRED.

테스트

    npx tsx --test src/lib/nickname.test.ts   24/24 PASS
    npx tsx --test "src/**/*.test.ts"         922/922 PASS
    npx tsc --noEmit                          src/ 오류 0
    npm run build                             Compiled successfully (ƒ /api/my/profile)

## 2026-09-12

### LOGIN RECENT VIEWED DENSITY UX V1 — 최근 본 단지 기본 5행

로그인(MY) 화면의 "최근 본 단지"가 서버가 준 목록을 전부 세로로 깔고 있었다.
API가 take: 20이라 최대 20행이고, 카드 한 줄이 약 86px이니 1,700px가 넘는다.
로그인 화면에서 아래 섹션에 닿으려면 계속 스크롤해야 했다.
...
기본 5행은 예전에 20행을 만들던 바로 그 배열의 앞 5건이다. 정렬(viewedAt desc)도,
보관 개수(take: 20)도, 중복 제거도, canonical 식별자도, 클릭 목적지(aptHref)도,
저장 동작도 그대로다. 새 API도 새 쿼리도 만들지 않았고 /api/my/recent 호출은 여전히
한 번이다. 목록이 이미 클라이언트에 다 있어서 펼치고 접는 데 조회가 끼지 않는다.

개수 규칙만 순수 모듈(src/lib/my/recent-rows.ts)로 분리해 직접 테스트한다.
apt-detail의 trade-rows.ts와 모양은 비슷하지만 합치지 않았다 — 실거래는 15씩 단계적
으로 늘리고(거래가 수백 건일 수 있다) 최근 본 단지는 한 번에 전부 펼친다(서버가
이미 20건으로 상한을 정한다). 규칙이 다르다.

홈 화면은 건드리지 않았다

"최근 본 단지"는 두 곳에 있는데 출처도 표현도 다르다. 홈은 localStorage(MAX_RECENT=8)
기반의 가로 스크롤이라 세로 압박이 없다. 지시대로 로그인 화면만 바꿨다.

행 스타일도 일부러 그대로 뒀다

.aptItem 패딩은 바로 위 "관심단지" 목록과 공유하는 클래스다. 여기를 줄이면 이 STEP
범위 밖 화면이 같이 바뀐다. 밀도 이득은 대부분 행 수에서 나오므로 공유 클래스를
건드릴 이유가 없었다. 테스트로 고정했다.

펼침 상태는 저장하지 않는다. 저장하면 다음 방문에 20줄로 다시 열려 이 변경이
무의미해진다.

밀도 효과(CSS 박스 계산, 행 ≈ 86px)

    20건(최대)  1,720px → 476px   −1,244px
    10건          860px → 476px     −384px
    6건           516px → 476px      −40px

브라우저가 없어 360/390/430px 실제 렌더는 확인하지 못했다 — DEVICE QA REQUIRED.

접근성: button 요소(키보드 기본 동작), aria-expanded, "최근 본 단지 더보기/접기"
라벨, 44px 터치 타깃.

테스트

    npx tsx --test src/lib/my/recent-rows.test.ts   16/16 PASS
    npx tsx --test "src/**/*.test.ts"               898/898 PASS
    npx tsc --noEmit                                src/ 오류 0
    npm run build                                   Compiled successfully
...
법무사 카드가 모바일에서 300px 가까이 차지해 단지 데이터보다 눈에 띄었다. 배지 +
큰 제목 + 긴 설명 + 48px 버튼 + 두 줄 면책문구 구성이었다.

    전                              후
    [광고·제휴]                     부동산 등기 상담
    부동산 등기 상담이 필요하신가요?   황보재호법무사사무실
    황보재호법무사사무실              매매 이후 등기 절차가 필요할 때
    매매 이후 등기 절차와 관련해...    상담할 수 있습니다.
    [카카오톡 상담]                  [카카오 상담] [전화 상담]
    [전화 상담 / 010-8026-4778]

"광고·제휴" 배지를 뺐다. 파트너 식별자·타입·추적 의미는 하나도 지우지 않았다 —
표시 변경이다. 이집이 제공하는 서비스가 아니라는 사실은 하단 면책 문구가 계속
말한다. (광고 표시 문구 제거는 표시광고 규제 관점에서 검토가 필요할 수 있어
문서에 주의 사항으로 남겼다.)

전화번호를 화면에서 뺐다. 보이는 건 "전화 상담"뿐이고 tel: 링크는 그대로다.
본문·버튼 라벨·aria-label 어디에도 숫자가 없다. 번호가 보이면 손으로 걸 수 있고,
그 통화는 어떤 집계에도 남지 않아 파트너에게 무슨 일이 일어났는지 말해줄 수 없다.

CTA 세로 쌓기 기준을 400px에서 339px로 내렸다. 라벨이 짧아지고 번호가 빠지면서
360px에서도 두 버튼이 나란히 들어간다(폭 합계 약 225px, 가용폭 약 300px).

중개사 파일럿

단지 상세에 롯데부동산중개사무소(심은희 공인중개사) 카드를 추가했다. 전화 한
채널뿐이고, 카카오·홈페이지·사진·로고는 존재하지 않으므로 넣지 않았다. 사진이
없는 자리는 lucide Store 아이콘 타일로 의도적으로 채웠다.

자격은 canonical 위치로만 판정한다. Production ApartmentMaster를 직접 조회해
부산 서구(sgg_cd=26140)의 법정동 19종을 확인했고, 영업 구역에 해당하는 9개만
정확 일치로 허용한다.

    서대신동1가(18) 2가(19) 3가(29)
    동대신동1가(6)  2가(6)  3가(8)
    부민동1가(4)    3가(1)
    부용동1가(7)                       합계 98개 단지

실측이 두 가지를 바꿨다. 부민동2가와 부용동2가에는 ApartmentMaster 행이 하나도
없어 목록에서 뺐다 — 없는 동을 적어두면 있는 것처럼 읽힌다. 그리고 서구에는
남부민동(단지 10곳)이 있어서, contains('부민동')로 매칭하면 영업 구역이 아닌
남부민동까지 잡힌다. 부분 문자열 매칭을 쓰면 안 되는 실제 사례다.

단지명은 보지 않는다 — getBrokerForLocation은 lawdCd와 dong만 받으며 이름을
넘길 자리 자체가 없다. '서구'라는 이름으로도 판정하지 않는다(대구·인천·광주·
대전에도 서구가 있다). 위치를 모르면 자격 없음이고, 자격이 없으면 카드가 렌더되지
않으므로 노출 집계도 발생하지 않는다.

실거래 밀도

기본 15행 → 5행. 더보기를 누르면 예전 기본값과 같은 15까지 펴지고, 15보다 많은
단지에서는 예전처럼 15씩 계속 볼 수 있다 — 밀도를 줄이면서 볼 수 있던 행을
잠가버리지 않는다. 접기가 새로 생겨 5로 돌아간다. 평형·기간·거래유형을 바꾸면
접힌 상태로 리셋된다(리셋 effect는 원래 있었고 값만 15에서 5로 바뀌었다).

거래 데이터는 하나도 바뀌지 않았다. 기본 5행은 예전에 15행을 만들던 바로 그
배열의 앞 5건이다. queryTrades / canonical aptSeq / 취소 제외 / 평형 매칭 /
정렬 / 가격 계산 / 차트 전부 그대로이고, 개수 규칙만 순수 모듈
(src/lib/apt-detail/trade-rows.ts)로 분리해 직접 테스트한다. 목록은 이미 전부
클라이언트에 있어 펼치고 접는 데 조회가 없다 — 거짓 "데이터 없음"이 스칠 수 없다.

밀도 효과(CSS 박스 계산, 실거래 1행 ≈ 60px)

    실거래 15건 이상   965px → 367px   −598px
    법무사 카드(360px) 346px → 197px   −149px
    중개사 카드        없음  → 250px   +250px (파일럿 구역만)

    순변화(360px)  파일럿 밖 약 −747px / 파일럿 안 약 −497px

브라우저가 없어 360/390/430px 실제 렌더는 확인하지 못했다 — DEVICE QA REQUIRED.

분석

partner_cta_impression / partner_cta_click 그대로. 두 카드가 같은 훅을 쓰고
partner_type으로 갈린다(legal_office / brokerage). 스키마 변경 없음, PII 없음.
번호를 감추면 클릭 관측률은 올라가지만 클릭은 여전히 클릭이다 — 통화 연결도,
상담도, 계약도 아니다. 그래서 이벤트 이름을 lead로 바꾸지 않았다.

테스트

    npx tsx --test src/lib/partners/broker-density.test.ts   38/38 PASS
    npx tsx --test "src/**/*.test.ts"                        850/850 PASS
    npx tsc --noEmit                                         src/ 오류 0
    npm run build                                            Compiled successfully
...
원인은 URL의 **내용**이었다. 비교 URL이 단지명·법정동·구코드를 전부 쿼리에 실었고
한글은 퍼센트 인코딩되므로 한 글자가 9자로 부푼다. 실측(현재 배포 오리진):

    진흥목화 vs 송암파크빌                292자
    해운대경동제이드 vs 대원아파트         303자
    해운대역푸르지오더원 vs 해운대경동제이드 340자

그 파라미터는 낭비가 아니라 당시에는 필요했다 — COMPARE_V2_PHASE2 주석이 적어둔 대로
복원에 쓰는 API가 전부 이름 기반이라 aptSeq만으로는 단지를 되살릴 수 없었고, aptSeq
조회 경로를 만드는 건 그 단계의 범위 밖이었다. 이제 서버가 aptSeq로 복원하므로 뗄 수
있게 됐다.

한 가지는 의심과 달랐다: text에 URL이 중복되고 있지는 **않았다.**
nativeShare({title, text, url})로 세 필드가 이미 분리돼 있었고 text는 URL을 담지
않았다. 말풍선의 덩어리는 URL 그 자체였다. 그래도 문구는 지정된 대로 맞추고 중복이
되살아나지 않도록 테스트로 고정했다.
...
새 계약
    ...
    진흥목화 vs 송암파크빌                292 → 77자
    해운대경동제이드 vs 대원아파트         303 → 80자
    해운대역푸르지오더원 vs 해운대경동제이드 340 → 81자  (−76%)
    e-jip.com 커토버 후                        → 57자

퍼센트 인코딩 문자 0개. 쿼리 키는 a, b 둘뿐. 이름·동·구코드·가격·사용자 입력 없음.

수신 측

새 서버 라우트 src/app/stats/compare/page.tsx(정적 세그먼트가 /stats/[type]보다
우선)가 aptSeq로 ApartmentMaster를 찾아 이름·구·동을 복원해 슬롯으로 넘긴다.
이름으로 되짚지 않는다(unique 키라 되짚을 이유가 없다). 형태부터 aptSeq가 아니면 DB에
묻지 않는다(score identity의 isWellFormedAptSeq 재사용). 해석 실패 시 비슷한 단지를
대신 보여주지 않고 "공유 링크의 단지 N곳을 찾지 못했습니다"라고 말한다. 두 자리는
독립이라 한쪽이 실패해도 다른 쪽은 정상 표시된다.

legacy 호환

이미 뿌려진 긴 링크는 계속 열린다(실측 200, 두 단지 정상 시드, 오탐 안내 없음).
다만 새로 만들지 않는다 — 열리는 순간 주소창이 짧은 형태로 정규화되고 이후 공유는
짧은 링크가 나간다. 긴 URL을 만들던 다른 두 곳(AI 검색 비교 진입, 상세→비교 링크)도
같은 빌더로 모았다. 예외는 검색 결과에 aptSeq가 없는 슬롯 하나뿐 — 그 슬롯만 동반
파라미터를 유지한다. 짧게 만들자고 열리지 않는 링크를 만들 수는 없다.

공유 payload

    title  A vs B 비교          →  A vs B 비교 | 이집
    text   이집에서 두 단지를…   →  2개 단지 시세와 데이터를 비교해보세요.
    url    주소창 복사+덧씌움     →  canonical 짧은 URL

useSharePage에 선택적 url을 더했다. 기본 동작(params)은 현재 주소창을 복사해 덧씌우는
방식이라 복원용 파라미터가 붙는 화면에서는 그 쓰레기까지 공유된다. 자기 canonical
링크를 아는 화면만 그걸 준다 — 다른 공유 호출부(통계 17종·지도·커뮤니티·AI 검색)는
url을 넘기지 않으므로 동작이 전혀 바뀌지 않는다.

OG / 도메인

/stats/compare의 generateMetadata가 a/b를 풀어 실제 단지명으로 만든다:
"해운대역푸르지오더원 vs 해운대경동제이드 | 이집" + canonical 짧은 URL. OG 이미지는
기존 buildOpenGraph 재사용(새 아키텍처 없음). 두 단지가 모두 확정될 때만 단지명을
제목에 쓴다 — "OO vs (알 수 없음)"을 만들지 않는다.
공유 URL 오리진은 window.location이 아니라 siteConfig에서 나온다. 컴포넌트에 호스트를
박지 않았고, 커토버 후 재배포하면 같은 코드가 e-jip.com 링크를 만든다.

프라이버시/분석: 공유 URL에 자유 입력이 들어갈 자리가 없다(인자가 aptSeq 둘뿐).
GA4 허용 파라미터는 utm_* 5개 그대로라 a/b는 GA4로 나가지 않는다. 스키마 무변경,
기존 share_success/share_attempt 이벤트 그대로.

한계: **카카오톡 실기기 확인은 하지 못했다**(브라우저/디바이스 도구 미승인). 확인한
것은 공유되는 URL이 짧고 인코딩된 한글이 0개이며 text에 URL이 섞이지 않는다는 데까지다.
"카카오톡에서 이렇게 보인다"고 주장하지 않는다.

검증: src 테스트 769/769 PASS · tsc src 오류 0 · 변경 파일 eslint 0 errors ·
npm run build exit 0(/stats/compare 라우트 등록 확인).

문서: docs/development/COMPARE_SHARE_URL_COMPACT_FIX_V1.md 신규.
...
- 지방세법 제11조(부동산 취득의 세율) — law.go.kr
- 지방세법 제151조(지방교육세 과세표준과 세율) — law.go.kr
- 농어촌특별세법 — law.go.kr
- 찾기쉬운 생활법령정보 "매매를 한 경우 세금 납부하기" — easylaw.go.kr

블로그·부동산 마케팅 페이지·계산기 사이트·커뮤니티는 사용하지 않았다.

결과: **세 산식 모두 법령과 일치. 고칠 부분 없음.**

| 항목 | 법령 | 확인된 산식 |
|---|---|---|
| 취득세 | 지방세법 제11조제1항제8호 | 6억 이하 1천분의 10 / 6~9억 (가액 × 2/3억원 − 3) × 1/100 / 9억 초과 1천분의 30 |
| 반올림 | 같은 조 | 소수점 다섯째자리 반올림 → 넷째자리(세율을 분수로 볼 때) |
| 지방교육세 | 지방세법 제151조 | 주택 유상거래는 **단서 적용** — 해당 세율 × 100분의 50으로 산출한 금액의 100분의 20 |
| 농어촌특별세 | 농어촌특별세법 제5조 | 과세표준 = 표준세율 100분의 2로 산출한 취득세액, 세율 100분의 10 → 가액의 0.2% |
| 농특세 비과세 | 같은 법 | 전용면적 85㎡ 이하 서민주택 |

대조 과정에서 확인한 두 함정 — 둘 다 구현이 이미 올바른 쪽이었다:

1. 지방교육세에는 산식이 **두 개** 있다. 일반 부동산은 (세율 − 1천분의 20) × 20/100이지만
   주택 유상거래에는 별도 단서가 적용된다. 일반 산식을 1% 구간에 쓰면 **음수**가 나온다.
2. 농특세는 "취득세액의 10%"가 **아니다.** 과세표준이 실제 세율과 무관하게 2% 기준으로
   고정이라, 1% 구간이든 3% 구간이든 결과는 가액의 0.2%다.

경계 검증(§6):

법령 산식을 코드와 **독립적으로 다시 구현해** 대조했다. 5억 / 6억 / 6억+1원 / 7억 / 8억 /
9억 / 9억+1원 / 10억 전부 일치. 6억·9억 경계에서 세율 도약 없음(6억 1%→6억+1원 1%,
9억 3%→9억+1원 3%). 5억~10억 구간 단조 증가 확인. 중간값 7억 1.67% / 8억 2.33%.

5억원 표본(§7) — 항목별로 분리해 확인:

    취득세      5억 × 1%              = 500만원
    지방교육세  5억 × (1% × 50%) × 20% =  50만원
    농특세      전용 84.95㎡ ≤ 85㎡     = 비과세
    합계                               = 550만원

작업:

- src/lib/finance-tools/acquisition-tax.ts — verifiedAgainstPrimarySource를 true로.
  ACQUISITION_TAX_RULE_VERSION.source를 실제 조문(지방세법 제11조·제151조,
  농어촌특별세법 제5조)으로 구체화. PRIMARY_SOURCES 신규 — 다음 검증자가 같은 곳을
  볼 수 있도록 출처 URL을 남긴다. 세 산식의 근거를 파일 상단 주석에 기록.
  **계산 로직은 한 줄도 바뀌지 않았다.**
- src/lib/finance-tools/finance-tools.test.ts — 신규 9건(총 39건).
  법령 산식을 코드와 독립적으로 구현해 대조하는 테스트, 경계 연속성/단조성,
  5억·10억 표본 항목별 검증, 농특세가 취득세액의 10%가 아님을 고정,
  85㎡ 경계(이하 비과세 / 초과 과세), UNSUPPORTED 경로에 금액 필드가 없음,
  PRIMARY_SOURCES가 1차 출처 도메인인지 검사
- src/app/tools/page.tsx + tools.module.css — 안내를 "적용 범위 / 예상 계산 /
  근거 조문·기준일·대조 완료" 구조로 정리(§8). 잘못된 로직을 면책 문구로 가리지 않고,
  좁은 적용 범위를 먼저 보여준다.
- docs 3종 갱신
...
- npx tsx --test src/lib/finance-tools/finance-tools.test.ts: 39/39 PASS
- npx tsx --test (src 전체): 708/708 PASS, fail 0
- npx eslint (변경 파일): exit 0
- npx tsc --noEmit: src/ 오류 0건(전체 exit 2는 기존 scripts//tmp/)
- npm run build: exit 0
- 렌더 실측: /tools 취득세 카드에 "근거: 지방세법 제11조·제151조, 농어촌특별세법 제5조
  (주택 유상거래·1주택 기준) · 기준일 2026-09-11 · 법령 원문 대조 완료" 노출 확인

상태 변경:

취득세 계산기 LIMITED → **READY(범위 한정)**. 적용 범위는 여전히 1주택·유상거래뿐이며
그 범위는 UI에 먼저 표시된다. 2주택 중과·생애최초 감면은 조정대상지역 데이터가 선행돼야
하므로 "출시 후"에 그대로 남는다.
...
.panel { padding: 2rem }에 **모바일 재정의가 아예 없었다**. 360px에서 카드마다 좌우
32px씩(합 64px = 화면 폭의 18%), 상하 32px씩을 그대로 썼다. 여기에
sectionBlock 32px + zoneTitle 16px이 겹쳐 섹션 시작에만 80px, 끝의 panel 하단
패딩까지 더하면 섹션 전환마다 112px이 내용 없이 소비됐다.
두 번째는 이미 쓸모없어진 여백 — quickButtons가 버튼 두 줄 시절의
marginTop 1.5rem + paddingTop 1.25rem + borderTop을 한 줄이 된 뒤에도 갖고 있었다.
세 번째는 항상 떠 있던 액션바가 .main에 예약시키던 calc(60px + 4rem).

밀도(전부 @media max-width:768px — 데스크톱은 1px도 안 바뀐다):

- .panel 상하 패딩 64px → 32px (-50%), 좌우 32px씩 → 16px씩
- .sectionBlock 32px → 20px (-38%)
- .zoneTitle 16px → 9.6px (-40%)
- .briefCard 64px → 44.8px (-30%)
- quickButtons 잔여 여백 44px → 16px (-64%)
- 실거래 행 상하 패딩 27.2px → 17.6px (-35%)
- 액션바 예약(숨김 시) 64px → 24px (-63%)

실거래 행은 데이터를 하나도 빼지 않았다(가격/증감/날짜/평형/층/거래유형 유지).
줄인 건 여백뿐이고 line-height를 명시해 기본값 슬랙을 걷었다. **행 전체로는 -16%**로
목표 20~25%에 못 미치는데, 행 높이의 약 70%가 글자 자체라 더 줄이려면 가격 글자를
줄이거나 메타 한 줄을 없애야 한다. 둘 다 §7/§17이 금지해서 멈췄다 — 수치보다 규칙.

지도 배치 — 예전 버튼 자리로:

직전 STEP은 위치 카드를 실거래 타임라인 **뒤**에 뒀는데, 사용자가 지도를 찾던 자리는
원래 2구역의 [지도][로드뷰] 버튼 자리(차트·투자지표 아래)였다. 그 자리에 버튼 대신
실제 지도를 놓는다. 옛 위치의 빈 래퍼/잔여 여백도 제거했다.
실측 오프셋: 위치 카드 11320 < 실거래 타임라인 12547 < 단지 상세 제원 14101 <
생활정보 15661. 중복 렌더 없음(카드 1개).

지도 높이 — 공용 컴포넌트를 안 건드리고:

KakaoMapEmbed는 아파트와 오피스텔이 함께 쓰므로 박혀 있던 minHeight:400px을 그냥
바꾸면 모든 소비자가 같이 바뀐다. CSS 변수로 열고(기본값 400px 유지) 아파트 위치
카드만 미디어쿼리로 재정의했다. 오피스텔은 변수를 설정하지 않아 400px 그대로다.
  <=375px 280 / <=400px 290 / <=440px 300 / <=1024px 340 / 그 외 380
지도와 로드뷰는 같은 프레임(inset:0)이라 높이가 항상 같다.

스마트 하단 액션바:

초기 진입·상중단 읽는 동안 숨김, 진행률 78%에서 슬라이드 업, 60% 이하로 **되돌아가야**
사라진다. 임계값이 하나면 경계에서 손가락이 몇 px 흔들릴 때마다 깜빡이므로 18%포인트를
벌렸다. 판정은 src/lib/apt-detail/action-bar-visibility.ts 순수 함수 — 스크롤 핸들러
안에 묻으면 검증할 수 없고, 히스테리시스는 이전 상태에 따라 답이 달라지는 규칙이라
특히 그렇다.
바가 뜰 때 .mainWithActionBar가 약 40px을 더해 문서가 길어지는데, 그로 인해 진행률이
떨어져 다시 숨는 진동이 가능한지 확인했다 — 수천 px 문서에서 40px은 약 1%이고 임계
간격은 18%포인트라 뒤집히지 않는다(테스트로 고정).
애니메이션 200ms ease-out, 바운스 없음, prefers-reduced-motion에서 전환 해제.
숨김 상태에서도 DOM에 남긴다(마운트 반복하면 슬라이드를 줄 수 없고 FavoriteButton이
상태를 다시 읽어야 한다). pointer-events:none + aria-hidden.
성능: passive 리스너 + requestAnimationFrame으로 프레임당 1회, 값이 실제로 바뀔 때만
setState → 스크롤 중 리렌더 없음. 새 API 요청 0건, 지도 재초기화 없음.

기능 회귀 없음:

관심단지/공유/글쓰기 동작은 하나도 바뀌지 않았다. visible prop 하나만 추가했고
즐겨찾기 상태·공유·analytics·글쓰기 라우팅·인증 가드 전부 그대로다.

DB / 스키마 / migration / 분석 스키마:

전부 변경 없음. 스크롤 진행률 analytics 이벤트는 추가하지 않았다(§21).

측정:

CSS에서 확정되는 값은 빌드 산출 CSS로 실측했다. 대표 단지 합산 절감은
패널6/섹션5/제목4/거래행15 기준 **약 619px**(390px 폰 가용 높이의 약 0.8화면).
**이 합계는 CSS 델타 × 등장 횟수 계산값이지 브라우저에서 잰 문서 높이가 아니다** —
document.scrollHeight 비교는 브라우저 계측이 필요해 하지 못했다.

검증:

- npx tsx --test src/lib/apt-detail/action-bar-visibility.test.ts: 15/15 PASS
- npx tsx --test (src 전체): 669/669 PASS, fail 0
- npx eslint (변경 파일): exit 0. 경고 1건은 apt-client.tsx의 기존 unused disable
- npx tsc --noEmit: src/ 오류 0건(전체 exit 2는 기존 scripts//tmp/)
- npm run build: exit 0
- 실데이터: 대신롯데캐슬(거래 0건·좌표 NO_MASTER → 사실대로 없음 상태),
  대연롯데캐슬레전드1단지(920건 긴 페이지), 흑교(1건 짧은 페이지) — 세 경우 모두
  위치 카드가 타임라인보다 앞, 액션바 초기 숨김, mainWithActionBar 미적용
- 모바일 QA는 STRUCTURAL ONLY(브라우저 렌더링 아님)

남은 한계:

모바일 QA가 렌더링 기반이 아니다(밀도는 결국 눈으로 판단해야 한다).
총 문서 높이 before/after를 브라우저에서 재지 못했다.
실거래 행은 -16%로 목표에 못 미친다(더 줄이려면 데이터/가격 위계를 건드려야 한다).
액션바 78%/60%는 실사용 관찰 없이 정한 값이라, 피드백이 나오면 상수 두 개만 고치면 된다.
...
### FINANCE TOOLS TRUST PATCH — 기존 금융 계산기 라벨/고지 정리

FINANCE FIT V1 PHASE 1 감사에서 발견된 취득세/DSR/LTV 계산기의 오해
가능성을 계산식 변경 없이 명칭·문구·고지만으로 해소. 대상: `/tools`
계산기 탭(취득세, "DSR"), apt-detail LTV 모달.

DSR 도구: "DSR 대출 가능 한도 추정" → "대출여력 간편추정"으로 개명
(실제 DSR을 계산하지 않으므로 "DSR" 표현 자체 제거). 결과 문구
"예상 대출 한도" → "간편 추정액"으로 변경, "실제 DSR 계산이
아닙니다" 상시 노출 고지 추가.

취득세 도구: "스마트 취득세 계산기" → "취득세 간편 추정"으로 개명,
반영되지 않는 조건(지역/면적/취득형태/생애최초 등) 상시 노출 고지
추가.

LTV 모달: 모달 키/제목 '대출한도' → 'LTV 기준 간편 추정'으로 변경
(해당 문자열이 모달 h2 제목으로 그대로 렌더링되는 구조 활용).
"대출 가능" 표현 제거, "실제 금융기관의 승인 가능 금액이 아닙니다"
명시 고지 추가.

**회귀 테스트 중 신규 발견(Phase 1 감사에서는 놓쳤던 부분)**: 취득세
ternary의 default branch 때문에 "무주택 (첫 매수)" 옵션이 실제로는
"3주택 이상"과 동일한 12% 최고세율로 계산되고 있었음 — 생애최초
우대와 정반대. 라벨 패치만으로는 안전하게 설명할 수 없는 결과라
판단해 작업 중단 후 사용자에게 보고, "무주택 옵션 임시 제거"로
결정(계산식 자체는 그대로 두고 UI 드롭다운에서만 제외 — 이번 STEP의
"계산식 변경 금지" 원칙 유지). 옵션이 사라진 이유를 설명하는 안내
문구 추가.

디자인: 기존 `--info-color`(#3b82f6, 이미 globals.css에 정의됨)
토큰 재사용한 중립 톤 disclosure panel — 공포성 빨간 경고 대신
정보성 파란 톤, `role="note"` + `aria-label`로 hover 의존 없이
접근성 확보.

숫자 회귀: 0건 변화(계산 결과 라인 자체가 diff에 전혀 등장하지
않음 — 표시 문구만 변경). 직접 재실행으로 before/after 6개 케이스
교차 검증.

모바일 QA: 390px/360px 실측(dev 서버, 실제 부산 단지 롯데캐슬로
LTV 모달 확인) — overflow 없음, 하단 네비 충돌 없음. 375px는
CSS 구조 변경이 없어 대표성으로 간주(resize_window 도구 한계로
독립 확인은 생략).

DB 변경: 없음. 신규 API: 0. 신규 analytics event: 0.

테스트/빌드: `npx tsc --noEmit`/`eslint` 신규 에러 0(기존 스크립트
오류만 잔존), `npm run build` 전체 성공.
...
- src/lib/map-property-focus.ts — 포커스 배타 규칙, 확대 단계별 밀도, 좌표 그룹핑,
  정확 identity 매칭(순수 로직, zero-import)
- src/lib/map-property-focus.test.mjs — 위 규칙 14개 테스트
- src/app/map/page.tsx — 라벨(단지→아파트), 오피스텔 파랑 체계, 포커스 모드,
  아파트/오피스텔 묶음 배지, 동일 좌표 선택 목록 카드, 오피스텔 검색 핸드오프,
  44px 터치 타깃, 하단 안내 문구 정리
- src/lib/officetel/search-read.ts — 검색 결과에 저장 좌표 추가(핸드오프용)
- src/components/ApartmentAutocomplete.tsx — 오피스텔 결과가 좌표를 그대로 전달
- src/lib/officetel/map-marker-contract.ts — OFFICETEL_MAX_ZOOM_LEVEL 이관(중복 제거)
- docs/development/MAP_UX_V2.md

서비스 기능 변경:

지도가 실제로 읽히게 됐다. Production 실측 기준, 부산진구 서면 360px 레벨 3에서
마커가 화면 면적의 80.0%를 덮던 것이 아파트 포커스 15.2% / 오피스텔 포커스 25.4%로
줄었다(마커 67개 → 9개 / 63개). 도로·지하철·동 경계·POI 라벨이 다시 보인다.

1) 포커스 모드: 아파트와 오피스텔은 더 이상 동시에 켜지지 않는다. 항상 정확히 하나가
켜져 있고(다시 눌러도 꺼지지 않는다) 기본은 아파트다. 학교/재개발/경공매/생숙은 기존
독립 토글 그대로다. 우측 칩에서 이 둘만 role="radio"로 노출한다.

2) 확대 단계별 밀도: 레벨 3 이하만 낱개 마커, 레벨 4~6은 묶음 개수 배지(누르면 확대),
오피스텔은 레벨 7 이상에서 그리지 않는다(데이터가 시군구 단위라 그보다 넓은 화면에서는
"이 지역에만 있다"는 잘못된 인상을 준다). 아파트에는 묶음 개념 자체가 없었는데 이번에
추가됐다 — 거래 데이터/가격 의미/식별자/상세 경로는 전혀 바뀌지 않았다.

핵심은 "낱개로 그린다"와 "이름을 쓴다"를 분리한 것이다. 레벨 3에서 오피스텔 63개를
낱개로 그리는 것 자체는 25.4%로 감당되지만, 같은 63개에 이름표(칩 폭 ~112px)를 붙이면
71.3%가 된다. 위치는 전부 보여주고 이름만 한 단계 더 확대해야 나온다.

3) 색: 오피스텔 teal(#0f766e) → 파랑(#1d4ed8). teal이 이집 Green과 너무 가까워 작은
화면에서 종류가 구분되지 않았다. 색만으로 구분하지 않도록 아이콘(Home/Building2)·
모양(둥근 말풍선/각진 사각)·가격 유무도 함께 다르다.

4) 라벨: 우측 "단지" → "아파트". 내부 state 키는 그대로다.

5) 동일 좌표 다중 master(32그룹/79건/최대 5개): 이제 낱개 5개를 겹쳐 그리지 않고 겹친
카드 모양의 개수 배지 하나로 그린다. 누르면 선택 목록이 열리고 각 항목이 표시명/동/
규모(호)와 함께 자기 master id로 이동한다. 데이터는 합쳐지지 않는다 — 픽셀 클러스터링
이전에 좌표가 완전히 같은 것만 묶으며, 반올림으로 묶지 않는다(가까운 다른 건물이
한 위치가 되는 것을 막는다).

6) 터치 타깃: 우측 레이어 칩 33px → 44px, 세로 간격은 줄여 컨트롤 열 높이 증가 최소화.

부수 발견 및 수정 — 오피스텔 검색 핸드오프가 안 되던 진짜 원인 2가지:

가. /map의 ApartmentAutocomplete에 includeOfficetel 프롭이 빠져 있어 기본값 false가
    적용됐고, **오피스텔 결과가 검색 목록에 아예 나오지 않았다** — placeholder는
    "아파트, 오피스텔 단지명 검색"이라고 약속하고 있었는데도.
나. 오피스텔 검색 결과는 lat:0, lng:0으로 내려왔다(검색 read가 좌표를 select하지 않음).
    지도는 이동할 좌표 자체가 없었다.

이제 검색 결과가 저장 좌표를 싣고 오고, 오피스텔을 고르면 오피스텔 포커스가 켜지고,
낱개 마커가 보이는 레벨까지 확대해 이동한 뒤, 도착한 마커 중 **officetelId가 정확히
일치하는 것만** 선택한다. 이름/부분주소/최근접 좌표/첫 마커로 대체하지 않으며 못 찾으면
아무것도 선택하지 않는다. 좌표가 없는 master(부산 8건)를 고르면 이동하지 않고
"위치 정보가 없어 지도에 표시할 수 없습니다"를 알린다.

stale/race: 레이어 전환과 검색 핸드오프가 반대편 레이어의 진행 중 요청을 순번으로
무효화한다. refreshActiveLayers는 focus를 인자로 받아, 같은 커밋 안에서 포커스를 막
바꾼 호출부가 낡은 layers 클로저를 읽어 엉뚱한 레이어를 조회하는 것을 구조적으로 막는다.
...
GET /api/search 응답의 officetels[] 항목에 latitude/longitude 추가(추가만 — 기존
소비처는 이 키를 무시해도 그대로 동작한다). 신규 라우트 없음.

검증:

- node --experimental-strip-types --test (map/officetel 9파일): 72/72 PASS
- npx tsc --noEmit: src 신규 오류 0 (저장소 전체는 FAIL_EXISTING_SCRIPT_ERRORS)
- npx eslint (변경 5파일): 0 problems
- npm run build: 성공
- 밀도 실측(부산진구 서면 360px): BEFORE 67마커/80.0% →
  아파트 포커스 9마커/15.2%, 오피스텔 포커스 63마커/25.4%, 레벨 2 이름표시 29마커/30.8%
- 가로 오버플로 0(360/375/390), 포커스 전환 시 반대 종류 마커 잔존 0
...
### MAP_LAYER_TOGGLE_V1 — 아파트/오피스텔 독립 토글 + 혼합 밀도 안전장치

작업:

- src/lib/map-property-focus.ts — 배타적 포커스 규칙을 독립 토글로 교체
  (togglePropertyLayer / ensurePropertyLayerVisible / isMixedPropertyMode /
  hasNoPropertyLayer), 혼합 모드 밀도 예산과 겹침 오프셋 상수 추가
- src/lib/map-property-focus.test.mjs — 21개로 확장(독립 토글 4조합, 검색 상태 보존,
  혼합 밀도, 좌표 그룹핑, 정확 identity)
- src/app/map/page.tsx — 독립 토글, aria-pressed 통일, 혼합 밀도, 혼합 겹침 오프셋,
  아파트 상태(로딩/성공/실패) 추가, 3가지 로딩 문구, 아파트 빈/실패 안내,
  검색이 반대편 레이어를 보존, 선택 identity 일시 de-grouping
- docs/development/MAP_UX_V2.md 후속 절, CHANGELOG.md, DECISIONS.md

서비스 기능 변경:

아파트/오피스텔이 학교·재개발과 같은 **독립 토글**이 됐다. MAP_UX_V2의 배타적 포커스
모드를 걷어냈다 — 그 목적이던 밀도는 확대 단계 규칙과 묶음 마커가 이미 해결했고,
배타성은 "둘 다 보고 싶다"는 요구를 막을 뿐이었다.

네 조합이 모두 유효하다: 아파트만(기본) / 오피스텔만 / 둘 다 / 둘 다 끔.
둘 다 끈 상태는 **정상 상태**로 취급한다 — 빈 지도 방지 로직도, "표시할 부동산이
없습니다" 안내도 두지 않고, 조회도 하지 않는다(요청 0건 실측). 별도의 "전체" 버튼은
만들지 않았다. 두 칩을 모두 켜면 그것이 전체다.

접근성: role="radio" + aria-checked(배타 그룹) → 여섯 칩 모두 aria-pressed(독립 토글).
실측으로 role="radio" 노드 0개 확인.

혼합 모드 밀도(§6): 단독 두 값을 그냥 더하면 15.2% + 25.4% ≈ 40%로 선호 목표 경계에
걸린다. 그래서 혼합일 때만 오피스텔 예산을 한 단계 조인다(낱개 한계 3→2, 이름 한계 2→1).
아파트 가격 칩은 그대로 둔다 — 화면을 나눠 써야 하면 정보가 적은 쪽을 먼저 묶는다는
MAP_UX_V2의 원칙 그대로다. 실측(부산진구 서면 360px 레벨 3): 혼합 38마커 / 28.8%
(아파트 9 + 오피스텔 묶음 29). 단독 모드 수치는 이전과 동일해 혼합 예산이 단독을
깎지 않는다.

혼합 겹침(§8): 혼합 모드에서만 오피스텔 오버레이를 16px 아래로 옮겨 아파트 말풍선과
같은 지점에 겹치지 않게 한다(좌표·식별자·클릭 대상 불변). 합성 Green+Blue 마커는
만들지 않았다 — identity 병합이 아니라 표시 위치 조정으로 충분했다.

검색(§9/§10): 고른 종류의 레이어는 반드시 켜되 **반대편은 건드리지 않는다**. 아파트만
켠 상태에서 오피스텔을 검색하면 둘 다 ON이 되고 고른 오피스텔이 정확히 선택된다(실측).
고른 오피스텔이 동일 좌표 묶음 안에 있으면 그 identity 하나만 묶음에서 일시적으로 꺼내
낱개 칩으로 그려 배지 뒤에 숨지 않게 한다 — 나머지 형제는 옆 배지로 계속 선택 가능하다.
혼합 모드에서는 확대 목표 레벨도 오피스텔의 조여진 한계를 쓴다.

부수 발견 및 수정:

아파트 레이어에는 상태 구분이 아예 없었다 — 조회 실패를 console.error로만 삼켜서
"불러오지 못함"과 "이 범위에 아파트가 없음"이 화면에서 똑같이 보였다(오피스텔은 V1부터
3상태를 갖고 있었다). 혼합 모드에서 한쪽만 실패하는 경우가 생기며 더 중요해져,
아파트도 idle/loading/ready/error를 갖고 실패 시 "아파트 정보를 불러오지 못했습니다.",
진짜 0건일 때 "현재 지도 범위에 표시할 아파트가 없습니다."를 구분해 띄운다.
배열이 아닌 응답도 조용히 무시하지 않고 실패로 처리한다(FAILED != EMPTY).
...
- node --experimental-strip-types --test: map-property-focus 21/21 PASS
- npx tsc --noEmit: src 신규 오류 0 (저장소 전체는 FAIL_EXISTING_SCRIPT_ERRORS)
- npx eslint (변경 2파일): 0 problems
- npm run build: 성공
- 밀도 실측(부산진구 서면 360px 레벨 3): 아파트 단독 9/15.2%, 오피스텔 단독 63/25.4%,
  혼합 38/28.8%, 둘 다 OFF 0/0% 및 요청 0건
- 검색 상태 보존 실측: 아파트만 ON → 오피스텔 검색 → 둘 다 ON + 한일오르듀 정확 선택
...
### MOLIT PARTIAL TRUST V2.1 — Production 시각 QA + 결함 2건 수정

작업:

V2 보고서에 "미완료"로 남겼던 시각 QA를 실제로 수행했다(설치된 Chrome을 playwright-core로
구동, 저장소 밖 scratchpad에 설치해 package.json/package-lock.json 미변경).
폭 360/375/390/767/959/1280, 배포된 Production 빌드 대상.

QA 시점에 실 partial은 이미 회복돼 있었다(202201/202204 정상 복귀, 60/60) — 회복 경로가
동작한다는 증거이기도 하다. 그래서 진짜 Production 응답을 받아 완전성 필드만 그날 실측된
값으로 되돌리는 결정적 fixture로 검증했다(제품 코드에 mock 없음, 값 생성 없음).

찾은 결함 2건(둘 다 표현 계층, 데이터 계약 변경 없음):

1. 상세 페이지에 같은 경고가 3번 쌓였다(PriceTrendChart 카드 / InvestmentMetrics 아래 /
   실거래 타임라인 위). 앞의 둘은 각 섹션을 설명하는 기존 문구이고 V2가 추가한 세 번째만
   중복이었다 — 이유를 이미 말하는 카드 두 개 바로 밑에서 같은 말을 반복했다.
   → InvestmentMetrics의 독립 배너 제거. 원본이 불완전하면 결합 계산값이 항상 억제되어
   카드가 스스로 이유를 말하므로 정보 손실이 없다. 결과: 섹션당 1회.

2. 비교가 다른 이유로 이미 접힌 분기에서 불완전 경고가 통째로 사라졌다.
   buildDifference()가 경고를 마지막 comparable 분기에서만 계산했는데, 조기 반환
   분기(데이터 없음/한쪽만 없음/면적 불일치)에서도 각 단지의 값은 화면에 그대로 뜬다.
   실측(/stats/compare, A만 부분 실패 + 면적 불일치): "면적이 달라 직접 비교하기
   어렵습니다."만 보이고 부분 실패 사실은 어디에도 없었다 — V2의 실제 회귀.
   → 진입부에서 한 번 계산해 모든 반환 경로에 싣는다.

수정 후 실측(6개 폭 전부): 가로 overflow 없음, 상세 배너 2회(섹션당 1회), 전세가율/갭
억제 2개, 관측값 단서 1개(매매가만 — 전월세 계열은 완전해서 전세가에는 안 붙는다, 계열별
판정이 화면에서 동작한다는 증거), failedMonths/내부 용어 노출 없음, 한글 문구 어절 단위
줄바꿈. 카드 높이 완전 69px → 부분 77~91px(폭발 없음).
/stats/compare, /stats/multi-compare, /stats/large-complex 모두 정상 표시.

완전 데이터 회귀: 네 카드 모두 실제 값(59.7%, 2.6억), 배너 0, 단서 0, 높이 69px 균일 —
V2 이전과 동일.

서비스 기능 변경:

부분 실패 시 상세 페이지 경고가 3회 → 2회로 줄어든다. 비교 화면에서 접힌 분기에도
불완전 경고가 노출된다. 완전 데이터 경로는 값/표시 모두 변화 없음.
...
- node --test --experimental-strip-types "src/lib/*.test.mjs": 291 tests / 290 PASS / 1 FAIL
...
/stats/multi-compare 차트는 원본 거래를 날짜 집계 없이 그려 선이 얽힌다. 완전 데이터에서도
동일한 기존 표현 품질 문제이고 완전성과 무관해 건드리지 않았다. 별도 STEP 후보.
...
### PERCEIVED PERFORMANCE V2.3 — MAP RENDER / CLUSTER OPTIMIZATION

작업:

- 지도 렌더 파이프라인 단계 계측 추가(`perfLog`/`perfNow`/`PERF_ENABLED`,
  `NEXT_PUBLIC_EJIP_PERF_DEBUG=true`일 때만 동작 — production 오버헤드 0)
- 병목 특정: 밀집 구 렌더 비용의 약 96%가 **오버레이 생성**이었다.
  부산진구 356개 마커 → 클러스터 293개를 전부 `CustomOverlayMap`으로 만드는데
  화면에 실제로 그려진 칩은 29개뿐 — 나머지 264개는 kakao `CustomOverlay`가
  화면 밖이라 DOM에 붙이지도 않으면서 생성·부착 비용만 냈다.
  (클러스터링 자체는 19~33ms로 전체의 4%였다 → 알고리즘은 건드리지 않음)
- 아파트 레이어에 뷰포트 컬링 적용(오피스텔 레이어가 이미 쓰던 개념 재사용,
  여유분 160px으로 통일). 선택/고정 마커는 `keepIds`로 컬링 면제

서비스 기능 변경:

- 화면 밖 아파트 마커의 오버레이를 만들지 않는다(표시 결과는 동일 —
  여유분 160px > 클러스터 반경 54px이라 보이는 개수 배지의 수는 컬링 전과 같다)
...
측정 (로컬 production 빌드, CPU 4x, 오버레이 커밋):

- 부산진구 360px 664.5 → 196.2ms (−70%), 1280px 775.0 → 356.4ms (−54%)
- 해운대구 360px 552.5 → 91.1ms (−84%), 1280px 511.1 → 254.3ms (−50%)
- 부산진구 z3 360px 690.8 → 112.4ms (−84%)
- 중구는 마커 51개로 컬링 대상이 거의 없어 변화 없음(1280px n=7 재측정으로 확인)
- 종단 usable: 밀집 구 −28~50%(부산진구 998→602ms, 해운대구 990→583ms @360px)
- production BEFORE(4G/4x): 부산진구 usable 4,250ms / 렌더 구간 1,736ms / LTmax 862ms

QA:

- §9 플로우 57/57 PASS (폭 360/390/430/1280 × 중구/부산진구/해운대구,
  직접진입·pan·zoom·레이어 ON/OFF·마커→상세→뒤로 복원·가로 오버플로 0px)
- identity: 선택 마커가 화면 밖으로 나가도 바텀시트/복원 URL의 aptSeq 유지 확인.
  구 경계를 넘을 때 선택이 풀리는 것은 컬링만 끈 빌드에서도 동일 → 기존 동작
- eslint 0 errors / `tsc --noEmit` src 0건(scripts·tmp는 기존 오류) / build 성공
- 지도 단위 테스트 53 pass / 1 fail(기존 ERR_MODULE_NOT_FOUND, 이번 변경 무관)
...
### PERCEIVED PERFORMANCE V2.6 — CRITICAL PATH BYTE REDUCTION

작업:

- 로딩 마스코트가 800x800 원본(82,492 B)인데 화면에는 96px(모바일 76px)로만
  그려지고 있었다(약 8배 과샘플링). 같은 그림을 256x256으로 재인코딩해
  13,912 B로 교체(-83%). 256은 데스크톱 96px@2x, 모바일 76px@3x를 모두 덮는다.
  표시 크기는 CSS가 고정하므로 레이아웃 시프트 없음(CLS <= 0.0011 실측).
- 자산 생성은 이미 설치된 sharp(Next 의존성)로 오프라인 1회 수행.
  런타임 의존성/package.json 변경/next-image 도입 없음. 원본 파일은 보관.
- fetchPriority="low" / decoding="async" 병기.

서비스 기능 변경:

- 없음(같은 그림, 같은 자리, 같은 표시 크기).
...
측정 (Production 통제, 390px / Slow 4G / CPU 4x / cold cache, 구별 n=10 valid,
cold 첫 run 제외 — BASELINE V1과 동일 방법론):

- 임계 바이트 293.3KB -> 226.4KB (-66.9KB, -23%)
- route JS 완료: 2,377~2,418ms -> 1,982~2,055ms (-348~-395ms)
- hydration:     2,631~2,708ms -> 2,303~2,333ms (-328~-390ms)
- usable: 중구 3,765->3,406(-359) / 부산진구 4,255->3,830(-425) /
          해운대구 3,841->3,447(-394)
- 폭별: 360px 4,302->3,840 / 430px 4,275->3,901 / 1280px 5,418->4,956
- 68.6KB / 1.6Mbps = 343ms 로, 실측 JS 단축(348~395ms)과 값이 맞는다.

정정:

- 구현 전 로컬 A/B(이미지 차단)에서 "JS가 안 빨라진다"는 결과가 나와 한때
  BASELINE V1의 인과 진단을 반증된 것으로 판단했고 커밋 e9a0ba9 본문에 그렇게
  적었다. 그 A/B가 틀렸다 — localhost + Playwright route.abort()가 실제 대역폭
  경쟁을 재현하지 못했다. 위 Production 전후 측정이 정답이며 BASELINE V1의
  진단이 옳았다. 상세는 V2_6 문서 §3.
...
- 로더 자산/깨진 이미지/마스코트 박스/CLS/가로오버플로/마커 렌더 24/24 PASS
  (360/390/430/1280 실제 폭 적용)
- V2.4 데이터 경로 6/6 PASS, keepIds/컬링 동일, SDK 단일 주입 유지
- eslint 0 errors / tsc --noEmit src 0건 / build 성공 / 지도 테스트 59 pass
...
## REPORT-5 — 일별 새로 확인된 실거래 (`/report/daily/[date]`)

- 관찰일(`created_at`) 기준 일별 리포트 추가. **날짜는 계약일이 아니라 이집이 거래를
  새로 확인한 날**이며, 계약일은 모든 거래 행에 따로 표시한다(제목·본문·푸터·KPI 4곳에서
  구분을 유지 — 스크린샷만 봐도 오해가 없게).
- `daily-observation.ts` — KST/UTC 경계, 백필 판정, 발행 상태 판정의 순수 단일 출처.
  백필 임계값은 배포된 sync 상수(`SALE_RECHECK_MAX_MONTHS_BACK`)에서 유도하며
  임의의 매직 넘버를 쓰지 않는다.
- 백필 의심일은 **그날 전체를 보류**한다(부분 차감 금지). 검증되지 않은 날은 0건조차
  말하지 않고 `PREPARING`으로 둔다 — 수집 실패를 0건으로 발행하지 않기 위해서다.
- 부산 현행 16개 코드만 집계한다. 2026-08-31에 관측된 서울/대구 파일럿 132행은
  부산 수치에 들어가지 않고 `READY_ZERO`(검증된 진짜 0건)로 발행됐다.
- 성능: 백필 보류일에 18만 행을 읽고 전량 버리던 경로를 제거했다. 관측창 풀스캔을
  1회로 줄이고(단일 집계 질의), 행은 발행 가능한 날에만 읽으며, 그 질의에는 집계에서
  얻은 `deal_date` 경계를 얹어 `(lawd_cd, deal_date)` 인덱스를 타게 했다(실측 191ms).
  2026-08-29 route warm 7,063~7,842ms → 3,247~5,337ms.
- **알려진 한계**: `apartment_trade_histories.created_at`에 인덱스가 없어(855k행/472MB)
  관측창 질의가 매번 풀스캔이다. warm 바닥이 1.3~1.9초라 §20 목표(warm ≤500ms)에
  미달한다. 해결에는 `@@index([createdAt])`가 필요하며 **schema 변경이므로 승인 대기**다.
  자세한 근거와 `ingestRunId`/`ingestMode` 권고는
  `REPORT_ENGINE_V1_REPORT_5_DAILY.md` §9·§10 참고.

## APT DETAIL — UNIT / TRADE FILTER BUG V1 (P0 데이터 신뢰)

- **버그**: Unit Master가 있는 단지에서 평형 칩을 눌러도 거래 데이터가 전혀 바뀌지
  않았다. 칩만 활성화되고 최근 실거래·타임라인·최고/최저·차트·전세가율은 로드 시점의
  84㎡ 기본값에 고정됐다 — 사용자가 A 평형을 선택하고 B 평형의 거래를 보는 상태.
- **원인**: 같은 면적이 두 문자열 도메인으로 존재한다. Unit Master는
  `"129.7178"`, 실거래는 `"129.7178m²"`(`api-molit.ts`가 붙인 suffix). 모든 필터가
  문자열 `===` 로 비교해 어떤 칩도 거래와 일치할 수 없었다. 이전 조치
  (DETAIL_TRADE_AREA_STATE_SPLIT_V1)는 상태를 둘로 쪼개 잘못된 데이터는 막았지만
  칩을 완전히 무력화한 채로 남겨뒀다.
- **수정**: 새 순수 모듈 `src/lib/unit-area-match.ts`가 두 도메인을 **숫자**로 잇는다.
  허용치는 지도/통계가 이미 production에서 쓰던 `AREA_MATCH_EPSILON`(0.001)을 그대로
  재사용했다 — 새 tolerance를 만들지 않았고, `84.6518` vs `84.6565` 같은
  micro-variant는 계속 분리된다. 이중 상태를 제거하고 모든 거래 위젯이 하나의 선택
  계약을 공유한다. 부수적으로 suffix 때문에 조용히 실패하던 평형 라벨 조회도 복구됐다.
- **동작**: 거래 없는 평형은 다른 평형으로 대체하지 않고 빈 상태를 표시한다.
  칩 전환은 네트워크 재조회 없이 로컬 필터링으로 처리된다(실측 API 요청 0회).
- **성능**: 칩 활성 p50 62ms / p95 84ms, 데이터 갱신 p50 64ms / p95 85ms.
- **QA**: 부산 3개 단지 × 360/390/430 = 칩 상호작용 93회, 가로 스크롤 0건.
  UI 건수가 동일 조건(12개월·취소 포함) DB 집계와 일치함을 확인했다.
- **함께 발견한 별개 결함(미수정)**: 상세페이지는 MOLIT 라이브를, 지도는 DB-first를
  읽는다. 그래서 DB에 있는 2026-09-05 거래들이 상세페이지에는 나오지 않는다
...
## REPORT-7 — 리포트 진입 UX / 최종 통합

- 리포트 엔진은 완성돼 있었지만 제품 안에서 **들어갈 길이 없었다**(URL을 알아야만
  볼 수 있는 상태). 6개 진입점과 허브를 연결했다.
- **route 모양의 단일 정의**: `src/lib/report/report-links.ts` 하나가 모든 리포트
  경로를 만들고, REPORT-6의 공유 canonical URL도 여기에 위임한다. 두 곳에서 각자
  조립하면 갈라지고, 갈라지는 순간 공유 링크가 다른 리포트를 가리킨다.
  **canonical identity가 없으면 null을 돌려주고, 호출부는 CTA 자체를 렌더하지 않는다.**
- 진입점: 단지 상세(`이 단지 한장 리포트`) / 지도 선택 카드(`한장 리포트`) /
  비교 CompareV2(`비교 리포트 보기`) / 통계 지역(`{구·동} 한장 브리핑`) / MY / 허브.
  각 CTA는 canonical id(aptSeq·lawdCd·dong)가 확정된 경우에만 나타난다.
  지역 CTA에는 `isBusanCurrentLawdCd` 가드를 걸어 부산 밖에서는 권하지 않는다.
- 단지 상세는 기존 `NextActionSection`에 `REPORT` 액션을 추가하는 방식이라 버튼 디자인
  언어와 클릭 트래킹을 그대로 물려받는다. `NEXT_ACTION_TYPES`(analytics 검증의 단일
  출처, schema 아님)에 값을 추가했고 타입 검사기가 라벨 맵 누락을 즉시 잡았다.
- **허브 `/report` 구현**(서버 컴포넌트). 부산 전체 브리핑과 일별 리포트는 자연스러운
  진입 화면이 아예 없었기 때문이다. 지역 칩은 검증된 16개 코드에서 그대로 나오고,
  단지·비교 리포트는 canonical aptSeq가 필요하므로 **어디서 여는지 안내만** 한다
  (허브에서 이름으로 단지를 고르게 하면 그게 이름 기반 식별이 된다).
- 일별 진입은 새 내비게이션 페이지를 만들지 않고 허브에 뒀다. 기존 `/stats/feed`는
  기간 기준 지역 피드라 REPORT-5의 관찰일 의미와 달라 문구를 맞추지 않았다.
  허브 문구는 "오늘 새로 확인된 실거래" + 계약일 구분 부제다.
- MY에는 허브로 가는 정적 링크 하나만 뒀다 — 열람 이력 저장소가 없으므로
  "내가 본 리포트"를 만들지 않는다(가짜 목록 금지).
- QA: 5개 폭(360/390/430/767/1280) × 전 여정 **201 checks PASS / 0 FAIL**,
  가로 오버플로 0건. 대신롯데캐슬 50평 회귀(6억 6,500만 / 2026.09.05 / 129.72㎡) 유지.
  접근성 기본 점검 통과(이름 없는 버튼 0, onclick div 0).
- 성능: CTA 탭 → 리포트 도달 378ms, 허브 528ms. **신규 의존성 0.**
...
## PWA INSTALL UX V1 — 홈 화면 설치

- 감사 결과 E-JIP은 **PWA가 아니었다**(manifest·service worker·theme-color 전부 없음).
  아이콘만 이미 준비돼 있었다. 설치 UX를 처음부터 붙였다. **신규 의존성 0.**
- manifest는 Next App Router 네이티브 라우트(`src/app/manifest.ts`)로 만든다.
  아이콘은 기존 브랜드 자산을 그대로 쓰고 브랜드를 새로 만들지 않았다.
- **설치 가능성을 실측으로 증명**했다. Chrome `Page.getInstallabilityErrors`가
  영속 프로필에서 **오류 0건**을 반환했고 `beforeinstallprompt`가 실제로 발생했다
  (임시 컨텍스트의 `in-incognito`는 테스트 환경 아티팩트).
- 서비스 워커는 **의도적으로 아무것도 캐시하지 않는다**. 설치 요건은 fetch 핸들러의
  존재로 충족하되, 캐싱으로 실거래/리포트가 낡은 채 굳는 것을 막기 위해서다.
- 설치 상태 판정은 `install-state.ts` 한 곳(순수 함수). 판정 순서에서 **인앱 브라우저를
  iOS보다 먼저** 본다 — 카카오 인앱은 iOS에서도 공유 시트로 홈 화면 추가를 할 수 없어
  순서가 반대면 불가능한 안내를 하게 된다(테스트로 고정).
- 플랫폼별: Android는 네이티브 프롬프트, iOS Safari는 공유 시트 3단계 안내,
  카카오/인앱은 외부 브라우저 안내. **카카오에서 원탭 설치를 주장하지 않는다.**
- 배너는 모바일 전용·설치 시 미노출·닫으면 **14일 쿨다운**(localStorage, 영구 억제 아님).
  MY의 진입점은 쿨다운과 무관하게 항상 남아 있다.
- **하단 바 충돌 결함을 하나 잡았다.** `Header`의 `.menuList`가 모바일에서 fixed 하단
  탭바가 되는데 표시가 없어 측정에 안 잡혔고, 홈/단지 상세에서 배너가 탭바를 덮고
  있었는데도 초기 QA는 통과했다. `data-bottom-bar` 표시를 추가해 배너가 런타임 실측으로
  피하도록 고쳤다(barTop 784 / bannerBottom 776, 8px 여유). 배너 z-index(900)는
  탭바(1000)보다 낮아 측정이 실패해도 탭바를 덮지 않는다.
- QA: 360/390/430 × 7개 화면 **21칸 전부 PASS**(겹침 0, 오버플로 0), 동작 매트릭스
  A~H 전부 PASS, 데스크톱 767/1280에서 모바일 배너 미노출.
  대신롯데캐슬 50평 회귀(6억 6,500만 / 2026.09.05) 유지.
- 분석은 기존 allowlist에 5개 이벤트만 추가(새 third-party 없음, payload 없음).
- **한계**: `/map` 충돌은 직접 검증하지 못했다(이 환경의 Kakao 키가 포트 3000 전용이라
  지도가 끝까지 렌더되지 않음). 실기기 검증 없음. 오프라인 미지원. maskable 아이콘 없음.

## REPORT TEMPLATE REDESIGN V1 — 인포그래픽 한장 리포트

- 리포트 6종의 **표현 레이어를 전면 재디자인**했다. 데이터 로직·trust 의미·DB/schema는
  건드리지 않았고, 템플릿은 계속 envelope만 읽는다(Prisma 접근 0, 재계산 0).
- 이전 출력물은 폭 520px짜리 세로로 긴 모바일 카드 더미였다. 저장/공유했을 때
  "리포트"가 아니라 "앱 스크린샷"으로 보였다. 화면이 아니라 **문서**를 기준으로
  폭·여백·위계를 다시 잡았다: 데스크톱 1000px 캔버스, 헤더 → KPI 스트립 →
  2단 본문 → 해석 → 신뢰 푸터.
- 디자인 시스템을 한 곳에 모았다(`ReportSheet.module.css` + `ReportPrimitives.tsx`).
  6종이 같은 파일을 쓰므로 스타일이 갈라질 수 없다.
- 적용: 단지 / 비교(최소 기준) + 지역 city·district·dong + 일별까지 4종 전부.
  두 종만 바꾸면 나머지가 옛 카드 스타일로 남아 제품이 어긋나 보인다.
- **데이터 진실성 유지**: MISSING은 흐린 '정보 없음'(0으로 채우지 않음), LIMITED/
  UNVERIFIED 칩 유지, 점수가 없으면 다이얼을 그리지 않고 "준비 중", 하이라이트에
  기간 문구(`최근 2년`) 동반, 평 라벨 없음, 비교 종합 승자 없음,
  일별 보류/준비중은 상태 박스가 주인공이고 숫자 영역 자체가 렌더되지 않는다.
- **표시 버그 하나 수정**: 비교 리포트가 준공연도를 `toLocaleString`으로 찍어
  "2,014년"으로 보였다. 연도 전용 포맷터로 `2014년`으로 고쳤다(값은 그대로, 표기만).
- **캡처 회귀 하나 발견·수정**: 새 디자인의 점수 도넛과 항목별 막대가 export에서 통째로
  사라졌다. REPORT-6에서 텍스트 잘림을 고치려 width/height 복사를 중단한 것이 원인으로,
  "크기 자체가 그림"인 요소가 0으로 찌그러졌다. 기존 `data-export-*` 계약과 같은 방식으로
  `data-export-fixed-size` 예외를 열어 해결했다.
- export/print: PNG 1080px 폭 유지, 액션바는 구조적으로 캡처 제외, 인쇄는 같은 문서를
  A4로 흘려보낸다(시트 전폭 + 액션바 display:none + PDF 생성 확인).
- QA: 5개 폭(360/390/430/767/1280) × 8개 리포트 변형 = **40칸 전부 PASS**.
  가로 오버플로 0, 금지 표현 0, 평 라벨 0, PNG 생성 248~472ms.
  테스트 147/147, src TS 오류 0, ESLint 0, build 성공.

## REPORT A4 EXPORT LAYOUT V1 — PDF 한 장 / 공유 이미지 A4 세로비

- 실기기 리뷰에서 나온 표현 결함 두 개를 잡았다. 데이터 로직·trust 의미·Score·
  DB/schema는 건드리지 않았고, envelope도 그대로다. **레이아웃/내보내기 전용 STEP.**
- **결함 1 — 단지 PDF가 2페이지였고 1페이지 아래 절반이 비었다.** 원인은 두 가지가
  겹친 것이다. (a) `.section`/`.footer`까지 `break-inside: avoid`로 묶여 있어서
  남은 자리에 못 들어가는 섹션이 통째로 2페이지로 점프했다. (b) 인쇄 밀도가 화면
  밀도 그대로였다 — `.sheet{font-size:10.5pt}`를 걸어도 자식이 전부 `rem`(루트 기준)
  이라 아무것도 줄지 않았고, 인쇄 폭 703px 때문에 모바일 media query가 걸려 KPI가
  2열 3행으로 펼쳐졌다. 실측 1661px / 예산 1032px = 161%.
- **결함 2 — 공유 PNG가 긴 웹 스크롤 캡처였다.** 보이는 시트를 그대로 구웠기 때문에
  360px 폰에서 공유하면 374×2429px짜리 세로 이미지가 나왔다.
- **웹 / 문서 분리**: 같은 envelope·같은 컴포넌트·같은 DOM을 쓰되 밀도만 셋으로
  갈랐다. 레이아웃의 변하는 값(열 수·방향·라벨 폭·줄 높이·여백·글자 배율)을 전부
  `.sheet`의 CSS 변수로 올려서, 반응형/인쇄/내보내기가 **구조 규칙을 복제하지 않고
  토큰만 덮어쓴다.** 별도 템플릿을 만들지 않았다 — 복제하면 곧 두 벌이 서로 다른
  사실을 말한다.
- **공유 PNG**: `captureReportExport()`가 화면 밖에 A4 폭(794px) 클론을 세우고
  `data-export-mode="a4"`를 켠 뒤 굽는다. 보이는 화면은 흔들리지 않는다.
  결과 **1080 × 1528 (1 : 1.415, A4 = 1 : 1.414)**, 뷰포트 폭과 무관.
  Web Share/카카오 공유도 같은 이미지를 싣고 canonical URL은 계속 동봉한다.
- **페이지 나눔**: 쪼개지면 곤란한 작은 단위(KPI 카드·거래 한 줄·인사이트 상자 등)만
  보호하고 `.section`/`.footer` 보호는 제거했다. 제목은 `break-after: avoid`로 본문과
  붙여 고아 제목을 막는다. 2페이지가 되더라도 1페이지가 먼저 채워진다.
- **압축 방식**: `transform: scale()`로 페이지를 줄이지 않았고 글자도 작게 만들지
  않았다. 여백·줄 높이·중복 배치를 줄였다(KPI 3열, 강점/확인할 점 2열, 항목별 점수
  2×2, 계약일 같은 줄). `--r-fs`는 큰/중간 글자에만 곱하고 캡션은 그대로 둔다.
  인쇄 본문 기준 섹션 제목 약 10.3pt / 값 9.5pt / 캡션 8.9pt.
- `@page` 여백 12mm → **10mm** (가용 703×1032 → 718×1047px).
- **거래 행 정책**: 문서에서는 4건까지 보이고, 5건 이상일 때만
  "최근 거래 일부 표시 · 자세한 내용은 이집에서 확인" 문구가 붙는다.
  면적·층·계약일·가격은 어느 경우에도 유지된다. envelope은 손대지 않았다.
- **채움률에 따른 세로 정책**: A4의 90% 이상이면 남는 높이를 섹션 사이로 나누고,
  65~90%면 푸터 위 한 곳에 모으고, 65% 미만이면 A4로 늘리지 않는다. 준비 중 상태
  일별 리포트(채움률 0.42)를 A4로 늘리면 화면의 60%가 흰 여백이 되기 때문이다.
- **결함 두 개 추가 수정**: PWA 설치 배너가 인쇄물에 남고 있어 `data-export-exclude`를
  붙였다. 비교 리포트 A/B 배지가 공유 PNG에서 세로 막대로 찌그러져
  `data-export-fixed-size`를 붙였다(크기 자체가 그림인 요소 계약).
- **결과(실측, 헤드리스 Chrome + CDP)**: PDF는 단지 963 / 비교 1019 / 시 959 /
  구 981 / 동 752 / 일별 431px — **6종 전부 1페이지**, 마지막 장 채움 92~97%(동 72%,
  일별 41%). 공유 PNG는 준비 중 일별을 뺀 5종이 전부 1080 × 1528.
- **웹 회귀 없음**: 단지 2429px / 비교 2156px / 일별 722px로 이전 커밋과 동일.
  시·구는 −6px인데, `.kpiValueLong`이 400px 이하에서 `.kpiValue`에 덮여 무력화돼 있던
  것을 특이도로 되살린 결과다(원래 의도대로 긴 KPI 값이 한 단계 작아진다).
  360/390/430/767/1280px 가로 넘침 0, 하단 액션바 여백 유지.
- 테스트 98/98, src TS 오류 0, 변경 파일 ESLint 0, build 성공. 이미지 생성 119~171ms.
