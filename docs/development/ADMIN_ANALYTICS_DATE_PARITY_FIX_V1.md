...
## 14. Production QA — 실제 관리자 세션으로 확인

배포 후 2026-09-21 16:42 KST, 브라우저에 이미 로그인된 관리자 세션으로 직접 확인.
...
| 방문 세션 | **317** | **317** | **0** |
| 페이지뷰 | **318** | **318** | **0** |
| fetchedAt | 07:42:47.824Z | 07:42:47.501Z | 0.3초 |

**화면 실측:**

| 화면 | 표시 |
|---|---|
| 관리자 대시보드 | 오늘 방문 세션 **317** / 오늘 페이지뷰(PV) **318** · "오늘 = 한국시간 00:00 기준 · 마지막 갱신 16:43" |
| 사용자 행동 분석 (오늘) | 방문 세션 **317** / 페이지뷰 **318** · "오늘 = 한국시간 00:00 기준 · 관리자 대시보드와 같은 기준입니다" |
| 사용자 행동 분석 (7일) | 362 / 452 — 달력일 계약 값(배포 전엔 rolling 367 / 513이었다) |

부가 확인: 행동 분석의 KPI "방문 세션 317"이 그 아래 "사용자 여정 → 방문 317"과
이제 **같다**. 예전에는 한 화면 안에서도 이 둘이 갈라질 수 있었다.

## 확인한 기존 이상 (이번 변경과 무관)

1. **`/admin/ops`가 500으로 떨어진다.** 이번 STEP은 이 라우트를 건드리지 않았고,
   `error_logs`가 진짜 원인을 이미 기록하고 있다(ADMIN_ERROR_LOGGING_P1이 의도대로 동작):
   `[ADMIN_OPS_FAILURE][PrismaClientKnownRequestError:P2024] prisma.apartmentTradeHistory.count()
   — Timed out fetching a new connection from the connection pool (timeout 10s, connection limit 1)`,
   기록 시각 06:26 KST(이번 배포보다 10시간 앞). 부분 실패 격리는 region-model 경로만
   감싸고 있어, 본 집계 쿼리가 pool에서 막히면 라우트 전체가 500이 된다. **별도 항목.**
2. **"인기 검색어 TOP 10"이 비어 있다.** 달력일 전환 탓이 아니다 — `search_logs`의 마지막
   행이 **2026-09-12**라 rolling이든 calendar든 최근 7일은 **둘 다 0건**이다(원천 확인).
