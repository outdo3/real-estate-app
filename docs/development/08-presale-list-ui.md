...
배포(`https://real-estate-app-park11.vercel.app`) 후 모바일 실기기에서 `/presales`를 확인한 결과 다음이 모두 정상 확인되었다.

- `/presales` 정상 접근, 1,046건 데이터 정상 로딩
- 지역/상태/가격 필터 및 복합 필터 정상 동작
- 모바일 카드 레이아웃 정상(가로 스크롤 없음)

배포 직후 일시적으로 "분양정보를 불러오지 못했습니다." 오류가 1회 보고되었으나, 이후 재현되지 않았고 production `/api/presales` 반복 호출(6/6) 전부 정상 응답을 확인해 API/DB 자체의 결함이 아닌 일시적 현상(배포 직후 캐시/콜드스타트 등)으로 판단했다 — 근본 코드 결함은 확인되지 않았다.

**PRESALE P2-D2(분양 목록 + 기본 필터 + 상태 UI)를 최종 완료로 승인한다.** P2-D3(상세페이지)로 진행한다.
