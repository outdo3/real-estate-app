...
## FINAL CASE — 636871 (26380:202601)

MOLIT 1셀 1회(캐시 없음) · SELECT만 · write 0. probe: `tmp/withdrawal-reappearance-check/probe-group.ts 636871 26380 202601`, raw 보존.

그룹: 일동지에닌 `26380-303` (괴정동) · 81.46㎡ · 34,000 · 2026-01-10 · 17층

| | occIdx | 상태 | cancelDate | registryDate | 원천에 있음 |
|---|---|---|---|---|---|
| DB 636773 | 0 | active | — | `26.04.02` | **있음**(자연키 정확 일치) |
| DB 636871 | 1 | **canceled** | `26.02.26` | — | **없음** |
| 원천 | 0 | active | — | `26.04.02` | — |

- 형제 수: 원천 **1** · DB **2**. 셀: 원천 273 · DB 274 — DB-only는 **636871 한 행뿐**, source-only 0.
- 다음 cron 모의: insert 0 · flip 0 · restore 0 · 보충 0 · 이 그룹 접촉 0 · **cancelReconcileSkipped 1**(이 그룹, SIBLING_COUNT_MISMATCH).
- 재버킷팅: 09-21 감사(`SIBLING_COUNT_MISMATCH_SINGLE_GROUP_AUDIT_V1.md` §재버킷팅)가 202512·202602에 0건임을 확인했다. 오늘은 1셀 범위라 인접 셀을 다시 보지 않았다.

**판정: STILL_MISSING.** 09-21과 09-22, 서로 다른 날의 두 조회에서 셀이 COMPLETE인데 같은 자연키가 없다 —
V1.1에서 제안한 기준(두 번 이상의 부재)을 충족하는 **유일한 withdrawal 후보**다. 영구 회수인지는 원천만으로 증명할 수 없다.

돌아온 3건과 모양이 다르다: 그 3건은 **9월 신고·미등기 active** 행이었고 등기가 붙자 돌아왔다. 이 건은 **1월 거래·2월 해제된 canceled** 행이고,
같은 조건의 active 행은 4월에 등기까지 끝나 원천에 그대로 있다. 즉 원천에서 사라진 것은 **해제된 옛 신고 기록**이다.

영향: 사용자에게 보이는 거래(636773, active·등기)는 원천과 정확히 같다. 남은 영향은 이 그룹의 **취소 동기화가 멈춰 있다**는 것 하나 —
636773이 나중에 해제돼도 cron이 반영하지 못한다(등기까지 끝난 거래라 가능성은 낮다).

### 정책 결정 권고 (결정은 사용자 몫)

**지금은 그대로 둔다(삭제·상태 변경 없음) + 관찰 유지.** 후보가 1건이고 사용자 화면 영향이 없으며, 해결하려면 `withdrawn` 상태 컬럼(스키마 변경, 승인 필요)이나
삭제가 필요하다. 같은 유형(해제된 옛 기록의 소멸)이 더 나오면 그때 `withdrawn` 상태 + reconcile이 그 행을 형제 수에서 제외하는 방식을 한 번에 승인받는다.
