...
## 22. 주택형 표시 최종 정책 (가독성 수정)

모바일 실기기 검수에서 주택형 요약 한 줄(`83.6488㎡ A · 공급 145세대 · 최고 7억 9,831만원`)이 정보 밀도가 높아 가독성이 떨어진다는 피드백에 따라 다음 정책으로 확정했다.

- **DB 원본 `supplyArea` 값은 절대 변경하지 않는다** — `formatArea()`는 표시 직전에만 `Math.round(v * 100) / 100`으로 반올림하고, 원본 컬럼/응답 값은 그대로 유지한다.
- **UI 표시는 소수점 최대 2자리**로 제한한다(`83.6488` → `83.65`).
- **불필요한 trailing zero는 제거**한다(`84.00` → `84`, `83.60` → `83.6`) — `Math.round` 후 JS `Number`의 기본 문자열 변환이 trailing zero를 자연히 제거하는 방식을 사용해, 별도 문자열 후처리 로직 없이 구현했다.
- 아코디언 요약을 **제목(면적+타입)과 공급·가격 정보 두 줄로 시각적으로 분리**했다 — 제목은 굵게/큰 글자(`accordionTitle`), 공급·가격은 보조 텍스트(`accordionSubtitle`)로 스타일을 구분한다.
- `<details>/<summary>` 아코디언 구조, 펼쳤을 때의 상세 정보(일반공급/특별공급/총공급/최고 분양가/원본 주택형 코드)는 변경 없이 그대로 유지했다.
- PC 폭에서도 별도 미디어쿼리 없이 동일한 두 줄 세로 스택 구조가 자연스럽게 표시됨을 확인했다(카드 폭이 넓어져도 어색해지지 않음).
- 코드/API/DB 변경 없음 — `presale-detail-client.tsx`의 표시 함수와 `page.module.css`의 아코디언 스타일만 수정.

---

## 최종 검수 결과 (2026-08-13)

모바일 실기기 검수를 완료했다. 전체 모바일 UX·기능(핵심요약/청약일정/주택형 아코디언/사업정보/위치 카드/청약홈 CTA/404·400·500 처리/목록↔상세 이동)이 정상 확인되었고, 주택형 표시 가독성 수정(§22)까지 반영해 최종 확인되었다.

**PRESALE P2-D3(분양 상세 + 일정 + 주택형 + 가격)를 최종 완료로 승인한다.**
