...
| 화면(부산 3개월) | 16 — 상위 20 밖(20위가 26건) | **46 (3위)** |
...
### 6개 기간 전체 재확인 (후속 요청, 2026-09-19 · 코드 변경 없음, 테스트 추가)

화면 `/api/stats/concentration` 상위 vs 브리핑 `/report/…?period=<키>` 렌더 결과(= 이미지/PDF 원본)의 "거래가 많은 단지" 상위 5.

| 범위 | 오늘 | 어제 | 7일 | 15일 | 30일 | 3개월 |
|---|---|---|---|---|---|---|
| 부산 전체 | 0 = 0 | 3,1,1,1,1 일치 | 9,4,4,4,4 일치 | 17,11,9,7,6 일치 | **46**,27,17,15,15 일치 | 53,52,**46**,42,40 일치 |
| 서구 | 0 = 0 | 0 = 0 | 4,2,1,1,1 일치 | 9,3,2,2,2 일치 | 15,5,4,3,2 일치 | 52,17,11,11,10 일치 |

12/12 건수 수열 일치 · 같은 건수 묶음 안 단지 집합 일치(동률 순서와 상위 5 경계에서 잘리는 동률 단지만 경로별 기존 규칙대로 다름). 15일 기간 진입 수정(`b7a304f`)과 충돌 없음. 화면 응답 101~486ms.

`complex-trade-count.test.ts`에 오늘·어제를 기간 루프에 추가하고, 6개 기간 × (화면 파이프라인 순위·건수 = 리포트 빌더 섹션 순위·건수, 단지 합계 = 요약 거래건수) 테스트(19)를 추가했다 — src 2,280 pass.
