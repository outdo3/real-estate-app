...
## 9.5. Production QA (배포 후, read-only)

커밋 `9dc73e1` push → Vercel Production 배포 후 `https://e-jip.com` 실측.

### 데이터 진실성 — 배포 전이면 불가능한 행 수

`/api/transactions?type=…&lawdCd=…&months=12`. **수정 전 상한은 12개월 × 1,000 = 12,000행**이었다.

| 지역 | 유형 | 행 수 | 고유 id | 에러 | partial | 성공 월 | 응답 |
|---|---|---|---|---|---|---|---|
| 서울 강남구 11680 | 전월세 | **22,211** | 22,211 | 0 | false | 12/12 | 9.2s |
| 서울 송파구 11710 | 전월세 | **19,111** | 19,111 | 0 | false | 12/12 | 8.9s |
| 경기 분당구 41135 | 전월세 | **14,731** | 14,731 | 0 | false | 12/12 | 5.1s |
| 부산 해운대구 26350 | 전월세 | 7,350 | 7,350 | 0 | false | 12/12 | 3.1s |
| 부산 해운대구 26350 | 매매 | 4,543 | 4,543 | 0 | false | 12/12 | 0.9s |
| 부산 서구 26140 | 매매 | 910 | 910 | 0 | false | 12/12 | 0.4s |

서울·경기 3개 지역이 모두 **12,000행 상한을 넘겼다** — 페이지네이션이 Production에서 실제로 동작한다는 직접 증거다. 중복 행 0(고유 id == 행 수), 부분 실패 0.

부산 3개 지역은 12,000 미만이라 수정 전후 동일하다(부산은 모든 월이 1페이지).

### 오류·부하 추이

| 창 | RATE_LIMIT | TIMEOUT | PAGING_PARTIAL | 합계 |
|---|---|---|---|---|
| 최근 2시간(배포 직후 + 위 QA 부하) | 0 | 0 | 0 | **0** |
| 최근 24시간 | 0 | 0 | 0 | **0** |

`error_logs` 최신 항목은 **2026-09-11**(배포 5일 전)이다. 배포 이후 새 오류가 한 건도 없다.

위 QA 자체가 서울·경기 12개월 × 다중 페이지 조회를 연속으로 돌린 부하였는데도 rate-limit·timeout·부분 실패가 0이었다 — 페이지 단위 게이트 통과가 초당 제한을 악화시키지 않았다는 실증이다.
