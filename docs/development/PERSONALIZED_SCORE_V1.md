...
### 6. Production QA (배포 `932c9f5`, 2026-09-15)

`/stats/compare?a=26350-9&b=26110-837`(롯데 vs 그린시티), 창 폭 1084px, 로그인된 실제 계정으로 **읽기·이동만**(저장 없음).

| 항목 | 결과 |
|---|---|
| 슬롯 | 롯데 / 그린시티(최대 2곳 그대로) |
| 공통 분석 | 제목 "이집 분석 (절대 평가 — 순위 아님)", 막대 교통 62·54 / 생활 37·71 / 교육 55·84 / 단지 52·53 — 같은 시각 score API `_shadowV2` 도메인 반올림 값과 **일치**, peer 줄 유지 |
| 개인화 블록 | peer 줄 바로 아래, 패널 안, 상태 `no_settings`("중요하게 보는 조건을 설정하면…" + [내 중요도 설정하기] 1개, 44px) |
| 요청 | `/api/my/preferences` 1회, 가로 넘침 없음 |
