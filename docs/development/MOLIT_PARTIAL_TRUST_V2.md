...
- 전세가율/갭: 억제 문구로 대체.
- 카드 아래 안내 배너 1개 추가. → **V2.1에서 제거했다**(상세 페이지 경고 3중 중복이
  실측으로 확인됨). 아래 "V2.1 — Production 시각 QA" 참고.
...
- **자연 발생 partial을 QA 시점에 재현하지 못했다.** 이 세션 초반에 실제로 관측된 적은
  ...
  결정적으로 고정했다.
- ~~시각 QA(360/375/390px)를 브라우저로 수행하지 못했다.~~ → **V2.1에서 수행했고 결함
  2건을 찾아 고쳤다.** 아래 "V2.1 — Production 시각 QA" 참고.
...
# V2.1 — Production 시각 QA

작업일: 2026-09-08 (V2 배포 직후)
대상: 배포된 Production 빌드 (`real-estate-app-phi-taupe.vercel.app`)

## 방법

V2 보고서에 "미완료"로 남겨둔 시각 QA를 수행했다. 브라우저 자동화가 없다고 적었으나,
설치된 Chrome을 `playwright-core`로 구동하는 방식이 가능했다 — 저장소 밖(scratchpad)에
설치해 `package.json`/`package-lock.json`을 건드리지 않았다.

QA 시점에 실 partial 케이스는 **이미 회복돼 있었다**(202201/202204가 정상 응답으로 복귀,
60/60). 회복 경로가 실제로 동작한다는 증거이기도 하다. 그래서 지시(§4)대로 결정적
fixture를 썼다: 진짜 Production 응답을 받아 **완전성 필드만** 그날 실측된 값
(`partial=true, failedMonths=[202201,202204], monthsSucceeded=58`)으로 되돌리고 그 두 달의
거래만 제거했다. 값을 지어내지 않았고, 제품 코드에는 어떤 mock도 넣지 않았다.

폭: 360 / 375 / 390 / 767 / 959 / 1280

## 발견한 결함 2건 (둘 다 표현 계층, 데이터 계약 변경 없음)

### 1. 상세 페이지에 같은 경고 3번

실측: PriceTrendChart 카드 안(y≈1984), InvestmentMetrics 아래(y≈2667), 실거래 타임라인
위(y≈3035)에 동일 문장이 쌓였다. 앞의 둘은 각자 자기 섹션을 설명하는 기존(V1) 문구이고,
V2가 추가한 세 번째만 중복이었다 — 이유를 이미 말하고 있는 카드 두 개 바로 밑에서
같은 말을 반복했다.

조치: InvestmentMetrics의 독립 배너를 제거했다. 정보 손실이 없는 이유는, 원본이
불완전하면 결합 계산값이 **항상** SUPPRESSED가 되어 두 카드가 스스로 이유를 말하고
(`일부 기간의 데이터가 없어 현재 계산할 수 없습니다.`), 관측값 카드에는
`일부 기간 미반영` 단서가 붙기 때문이다. 결과: 섹션당 1회 = 2회.

### 2. 비교가 접힌 분기에서 불완전 경고 유실

실측(/stats/compare, A만 부분 실패 + 면적 3㎡ 초과 차이):

```
가격 (84.9585㎡ 기준)
6억 5,000만        27억 5,000만
면적이 달라 직접 비교하기 어렵습니다.        ← 이것만 보였다
```

`buildDifference()`가 불완전 경고를 **마지막 comparable 분기에서만** 계산하고 있었다.
조기 반환 분기(데이터 없음 / 한쪽만 없음 / 면적 불일치)는 `caution: null`을 돌려주는데,
그 분기들에서도 각 단지의 값은 화면에 그대로 뜬다 — 즉 부분 실패로 얻은 가격이 완전한
데이터처럼 읽혔다. V2의 실제 회귀다.

조치: 경고를 함수 진입부에서 한 번 계산해 **모든 반환 경로**에 싣는다. 회귀 테스트는
`buildDifference` 본문에 `caution: null`이 하나도 남지 않았음을 검사한다.

## 수정 후 실측 결과

| 항목 | 360 | 375 | 390 | 767 | 959 | 1280 |
|---|---|---|---|---|---|---|
| 가로 overflow | 없음 | 없음 | 없음 | 없음 | 없음 | 없음 |
| 상세 배너 수 | 2 | 2 | 2 | 2 | 2 | 2 |
| 억제 카드(전세가율/갭) | 2 | 2 | 2 | 2 | 2 | 2 |
| 관측값 단서(매매가만) | 1 | 1 | 1 | 1 | 1 | 1 |
| failedMonths 노출 | 없음 | 없음 | 없음 | 없음 | 없음 | 없음 |
| 내부 용어 노출 | 없음 | 없음 | 없음 | 없음 | 없음 | 없음 |

카드 높이: 완전 69px → 부분 77~91px. 폭발 없음.

**전세가에 단서가 붙지 않은 것이 정상이다** — 그 계열(전월세)은 완전했다. 계열별 판정이
실제 화면에서 동작함을 보여주는 증거다.

- `/stats/compare`: 면적 불일치 문구 + `첫 번째 단지는 일부 기간의 거래 정보를 불러오지
  못해, 완전한 비교가 아닙니다.` 두 줄 모두 노출, 전 폭에서 자연 줄바꿈.
- `/stats/multi-compare`: 배너가 해당 단지명을 지목하고, 범례가
  `대신푸르지오2차 (일부 기간 미반영)`로 표시. 완전한 계열은 무표시.
- `/stats/large-complex`: 배너가 "최근 매매" 칸으로 범위를 한정해 정상 노출.

**완전 데이터 회귀**: 네 카드 모두 실제 값(59.7%, 2.6억), 배너 0, 단서 0, 카드 높이 69px
균일 — V2 이전과 동일.

## 범위 밖으로 남긴 관찰

- `/stats/multi-compare` 차트는 원본 거래를 날짜 집계 없이 그대로 그려 선이 스파게티처럼
  얽힌다. 완전 데이터에서도 동일하게 나타나는 **기존** 표현 품질 문제이고 완전성과
  무관해 건드리지 않았다. 별도 STEP 후보.
- 상세 페이지의 남은 배너 2개는 각각 차트/목록 섹션을 설명하는 기존 문구로, 중복이
  아니라 섹션별 안내로 판단해 유지했다.
