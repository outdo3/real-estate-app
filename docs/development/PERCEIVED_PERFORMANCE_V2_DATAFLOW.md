...
1. **배포 후 Production 검증** — §8의 5개 항목(Kakao 의존)을 실제 도메인에서 확인.
2. **버스 durable 캐시** — DB/schema 승인 STEP. §6.6이 근거.
3. **`/score`, `/info` 재진입 캐시** — §5와 같은 안전 규칙(실패·부분실패 미캐시)을 재사용.
