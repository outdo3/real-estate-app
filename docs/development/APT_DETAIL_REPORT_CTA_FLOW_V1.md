...
## 7. Production QA (배포 `76838f7`, 2026-09-15)

브라우저 창 폭 767px(이 환경 Chrome 최소 폭), 로그인 없음, 쓰기 없음.

| 단계 | 결과 |
|---|---|
| 상세(우동 롯데 `26350-9`) | 리포트 카드 렌더, 페이지 내 `/report/apt/` 링크 1개(카드 버튼), 상단 `이 단지 한장 리포트` 버튼 없음 |
| 지도 → 단지 선택(중구 대청동2가 그린시티, aptSeq `26110-837` 보유 — 이전이면 리포트 버튼이 보였을 단지) | 하단 카드: 단지명·동·가격·닫기·`상세보기`(48px, 카드 폭) — **리포트 문구·버튼 없음** |
| 상세보기 | `/apt/그린시티?lawdCd=26110&dong=대청동2가&aptSeq=26110-837` — 같은 canonical aptSeq 유지 |
| 상세 스크롤 | 리포트 카드 문서 높이 대비 63%, 링크 `/report/apt/26110-837` |
| 카드 클릭 | `/report/apt/26110-837` "그린시티 단지 리포트 - 이집" 정상 렌더, 공유하기·이미지·PDF 액션 표시 |
| 뒤로가기 | 같은 상세(aptSeq 유지)로 복귀, 카드 유지 |

참고: 우동 롯데 상세의 "지도에서 주변 단지와 보기"는 좌표 없이 기본 지역으로 열렸다(기존 동작, 이번 변경과 무관).
Production 스크린샷은 지도·리포트 렌더 중 캡처가 시간 초과되어 남기지 못했다(DOM 측정으로 대체).
