...
## 6. Production QA (배포 `06cf0b2`, 2026-09-15)

수정 전 재현과 같은 조건(GPS 없음). 클릭 후 약 12초 뒤 확인. 모든 경우 navigation entry가 `/map`(전체 로드).

| 경로 | 선택 | 최종 지도 URL | 마커 요청 | 안내 | ipinfo |
|---|---|---|---|---|---|
| 홈 | 부산 연제구 연산동 | `lat=35.1825602536452&lng=129.085536368165&lawdCd=26470` | 26470 | 없음 | 없음 |
| 홈 | 부산 강서구 명지동 | `lat=35.1066795355093&lng=128.915171586425&lawdCd=26440` | 26440 | 없음 | 없음 |
| 빠른 검색(연산자이 상세) | 부산 해운대구 우동 | `lat=35.1727271517301&lng=129.148399576019&lawdCd=26350` | 26350 | 없음 | 없음 |
| 빠른 검색(연산자이 상세) | 부산 북구 화명동 | `lat=35.2357636793361&lng=129.013861621487&lawdCd=26320` | 26320 | 없음 | 없음 |

- 좌표는 수정 전 재현 때 검색이 만든 좌표와 동일(매핑 무변경), 마커는 선택한 구 코드로 1회 요청.
- 뒤로가기(홈): 지도 → back 1회 → 홈(bfcache 복원, 검색창에 "부산 연제구 연산동" 유지). history 항목 1개 추가(중복 없음).
- 뒤로가기(빠른 검색): 지도 → back 1회 → `/apt/연산자이?…aptSeq=26470-1066` 상세, 모달 닫힘. 빠른 검색이 열릴 때 쌓은 같은 URL 항목(`ejipQuickSearch`) 위로 돌아온다 — 수정 전 `router.push`에서도 남던 항목이며 이번에 바꾸지 않았다.
- 회귀: `/map` 직접 진입은 기존과 같이 GPS 실패 → ipinfo → 26110 + 안내(마커 26140→26110). 홈에서 단지 결과(연산자이) 선택은 기존대로 `/apt/연산자이?lawdCd=26470&…&aptSeq=26470-1066` 상세로 이동.

## 7. 알려진 문제 / 다음

- 빠른 검색은 열릴 때 같은 URL history 항목을 하나 쌓는다(뒤로가기 = 닫기). 지역 선택으로 지도로 가면 그 항목이 남아, 상세로 돌아온 뒤 한 번 더 back해야 이전 페이지로 간다 — 수정 전과 같은 기존 동작(범위 밖).
- 이 환경에서 Kakao SDK 미로드(좌표 0,0) 상태는 Production에서 재현하지 않았다 — 단위 테스트(§5-7)로만 확인.
